...
**Ejemplos resueltos:** el system prompt del verificador (`verificador.py`, v5) incluye una sección de EJEMPLOS RESUELTOS con 3 casos reales de run_1/run_5 más 1 ejemplo negativo ("error frecuente — no hacer"), todos en vocabulario v2 (pares `{sintoma_capa1, causa_capa2}`) y nunca de run_3 (gate) ni de los casos del dev set. Viven en el prompt, no acá. Desde v5 el prompt ENSAMBLA esta taxonomía en runtime leyendo este archivo — pero SOLO el rango que va del encabezado de Capa 1 al de Las tres piezas, exclusive (Capa 1 con precedencia, tablas de Capa 2 y árbol de decisión; ver `taxonomia_section()` en `verificador.py`): editar ese rango ES editar el prompt. Las piezas de evidencia y la atribución múltiple —incluida la regla de jerarquía v2.6— NO viajan por referencia: viven en el bloque fijo del prompt de `verificador.py` (corrección documental v2.6.1).
