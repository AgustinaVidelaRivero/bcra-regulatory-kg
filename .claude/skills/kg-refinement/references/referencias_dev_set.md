...
- **Causas lado agente presentes solo como 2 secundarias (actualizado 2026-07-14):** CQ-017 (`alucinacion_agente` modo b — la fusión "operador de cambio = entidad financiera autorizada") y CQ-031 (`navegación`, ítem 4.6). Ninguna mueve el acierto. Las que eran modo (a) —las 4 glosas de CQ-017 y el claim de situación normal de CQ-019— se re-adjudicaron como falsos positivos del juez por la auditoría de truncamiento.
- **Falsos positivos del juez documentados dentro del set: 17 claims** (CQ-017: 5 · CQ-020: 7 · CQ-019: 2 · CQ-024: 1 · CQ-031: 2). Útiles para calibrar la disciplina de "sin par / no es defecto del sistema".
- **Sin primaria lado agente — límite conocido:** el dev set no ejercita el caso donde lo que rompe la respuesta es del agente (lado agente presente solo en las 2 secundarias de arriba). Consistente con el gate (`casos_control.md`, cuya cobertura de lados declara lo mismo) y **consignado para la reunión de mentores** (queda abierta la incorporación futura de un caso con primaria lado agente).
