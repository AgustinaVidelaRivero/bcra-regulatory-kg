...
- **Pata 1** ("qué deudores no deben ser objeto de clasificación") — **MIXTA, DOS PRIMARIAS** (re-adjudicada por la autora, 2026-07-14, motivada por la iteración 1 del verificador v5):
  ...
  - **`{context_recall, navegación}` PRIMARIA — ítem 4.6:** el edge `ope_clasificacion_de_deudores --no_está_sujeta_a--> ope_creditos_cedidos_sin_responsabilidad_para_el_cedente` (prov. p.15) fue **visible en runtime** en el output completo del paso 15 (`harness.py` pasa outputs completos al agente y almacena truncados en la traza) y no fue explotado.
  - **Nota de re-adjudicación (autora, 2026-07-14):** reemplaza la primaria única `{context_recall, alcanzabilidad_kg}` del 2026-07-13. Motivada por la iteración 1 del verificador v5 y el micro-hecho del paso 15; el ítem 4.5 sigue `alcanzabilidad_kg` porque los `ver_vecinos` completos de los pasos 5 y 6 NO exponen ningún portador (re-ejecutados: NINGUNO de los 4 ids portadores aparece) — solo el paso 15 expuso uno, y es el del 4.6. Aplica la prueba de alcanzabilidad ex ante de `taxonomia.md` v2.1.
...
- **Primarias por caso (actualizado 2026-07-14):** lado grafo en los 5 casos — `alcanzabilidad_kg` ×2 casos (CQ-031 pata 1-ítem 4.5 y pata 2, CQ-017) y `contenido_kg` ×3 casos (CQ-020, CQ-019, CQ-024) — más **una primaria lado agente** en la pata 1 de CQ-031 (ítem 4.6: `{context_recall, navegación}`, re-adjudicación del 2026-07-14). CQ-031 pata 1 es MIXTA (dos primarias): el acierto exige detectar ambas (regla de "varias primarias" de `casos_control.md`).
- **Causas lado agente:** una primaria (CQ-031 ítem 4.6, `navegación`) y secundarias en CQ-017 (`alucinacion_agente` en sus dos modos: b —glosa de cosecha propia— y a —portador exhibible no consultado—) y CQ-019 (modo a).
...
- **Frontera lado agente parcialmente cubierta (actualizado 2026-07-14):** el dev set ahora ejercita UNA primaria lado agente (`navegación`, CQ-031 ítem 4.6); el gate (`casos_control.md`) sigue sin primaria lado agente — **consignado para la reunión de mentores** (queda abierta la incorporación futura de un caso con primaria lado agente en el gate).
