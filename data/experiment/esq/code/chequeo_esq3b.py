"""
chequeo_esq3b.py — Chequeo de conformidad de una extracción contra el esquema
RETOCADO de U-ESQ-3b.

QUÉ ES Y QUÉ NO ES. Esto NO reemplaza ni modifica `validador_e1.py` (módulo de
producción, intacto): es un chequeo propio de la unidad, con la matriz
`prompt_esq3b.DOMAIN_RANGE_RETOCADO`, cuyo único destino es el REPORTE del
ejecutor (conteos de tipos nuevos emitidos, firmas que el esquema retocado
acepta o rechaza, anomalías). No se usa para armar las fichas —las fichas
muestran la extracción CRUDA, patrón de ESQ-2— y no adjudica nada.

Alcance deliberadamente acotado a lo que el reporte necesita:
  - entidades: `type` dentro del vocabulario retocado, `punto` dentro de los
    admitidos del chunk.
  - relaciones: `predicate` dentro del vocabulario retocado, `punto` admitido y
    firma (dominio, predicado, rango) contra la matriz retocada, con el
    pseudo-tipo "Sujeto" en el extremo de `aplica_a` / `ejecuta` (mismo criterio
    que producción).
No replica la normalización de provenance ni las advertencias de flags: eso es
del validador de producción y esta unidad no lo necesita.
"""
...
import comun_esq3b as cc      # noqa: E402,F401  (sys.path de e1_extractor)
...
def chequear(tool_input, chunk: dict) -> dict:
    """Devuelve conteos y rechazos por elemento contra el esquema retocado."""
    out = {
    ...
    for i, e in enumerate(entities):
        ...
        if etype not in pr.ENTITY_TYPES_RETOCADO:
    ...
    for i, r in enumerate(relations):
        ...
        if pred not in pr.PREDICATES_RETOCADO:
