# Nomenclatura canónica de los grafos de EV2 (y sus releases)
...
| **KG-Reextraído-r1** | `r1`, `salida_r1`, "KG-Reextraído release 1" | `data/experiment/reextraccion_v2/corpus_v2/salida_r1/kg.json` | `0226e9477baee02d772bbfecee78a49441b189d0e0512ca5e22956dfb084196a` | `185e042` (2026-08-23, cierre de U-B1a) | **Gen. 3 + E4/E5 determinísticos** — mismos crudos de E1/E3 de KG-Reextraído (sin API), re-ensamblados por `corpus_v2/ensamblar_r1.py` con: invariantes y guarda de merge cross-TO (solo `Sujeto` de catálogo se fusiona; 5 colisiones de contenido registradas sin fusionar), E4 determinístico (43→44 propuestos: 3 resueltos por alias declarado, 41 en cuarentena con `padre_sugerido` flaggeado; TextoOrdenado 6→5), E5 esqueleto (paridad 70/82 con KG-Refinado), referencias cruzadas norma→norma (5.645 aristas `referencia` con evidencia), provenance rica (`chunk_id`, `paginas`, `ancestros`), cola humana flaggeada (79 unidades ingresan con `estado_e3`). **6.529 nodos / 17.772 aristas, 16 tipos de relación** *(fe de erratas 2026-08-23: esta fila y el mensaje del commit `185e042` decían «23», copiado de la propuesta de cierre de U-B1a sin recomputar; el conteo verificado sobre `kg.json` es 16 — los 11 del grafo sellado más `subclase_de`, `miembro_de`, `instancia_de`, `parte_de` y `padre_sugerido`; detectado por U-B1.8, regla d)*. Tests T1–T7 7/7; doble corrida byte-idéntica. Es el grafo sellado que se evalúa (principio 9 del plan); la release r2 será la versión post-evaluación. |
