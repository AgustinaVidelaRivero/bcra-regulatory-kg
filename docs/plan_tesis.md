...
ESTADO 11/09 — GATE DEL CAPÍTULO: «capítulo enviado 11/09, esperando validación escrita de los
mentores». (registrado por instancia del plan 11/09; firmado por la autora 14/09). Capítulo 3 completo (§3.1–§3.10, `main.tex:546–1029` en `ed389c4`) y
§4.1, §4.2, §4.6 (`main.tex:1033`, `:1042`, `:1063`) commiteados y pusheados en `ed389c4`; PDF
enviado a los mentores el 11/09 para la validación por escrito acordada el 04/09
(`docs/registro_reunion_mentores_2026-09-04.md` §1). **Nada del escalado (B6) se corre hasta
que la validación esté registrada.** **RUTA CRÍTICA → validación escrita de los mentores.**
Tres decisiones ABIERTAS, dependientes de la respuesta: (1) los ocho límites de la Tabla 4
(`tab:limites`, `main.tex:919–1029`) con sus destinos; (2) el principio de gobierno de la
enmienda (`data/experiment/esq/laudo_esquema_congelado.md` §1, `:12–:20`); (3) el riesgo
15/157 declarado al final de §3.10, con la alternativa de un test sobre documentos ajenos a
los 15 antes de escalar. Dos ítems ABIERTOS: (a) la tabla §3 del laudo congelado
(`laudo_esquema_congelado.md:90–:102`) tiene 9 filas sin columna de grupo; la Tabla 4 de la
tesis tiene 8 límites — seis coinciden fila a fila, el límite 1 viene de `:88` y las filas
`:99`, `:101` y `:102` no están en la tesis; el caption de la Tabla 4 ya no afirma un número
para los límites del ensamblado. Clasificar las nueve filas es una ADENDA AL LAUDO FIRMADA POR
LA AUTORA, no una edición de instancia. (b) la fila de U-COB-A (`plan_tesis.md:647`, numeración actual) dice que
el bloque A «reemplaza **11**» unidades degeneradas; `alcance_nueve.json:233–236` dice «9324 -
10 + 77 = 9391» y §4.2 de la tesis (`main.tex:1045`) dice «reemplazan a las diez». NO se
corrige aquí: PENDIENTE DE LAUDO DE LA AUTORA. (Hallazgos registrados por verificación,
Mandato 2, 11/09.) PASE DE RECONCILIACIÓN PLAN↔GIT (registrado por instancia del plan 11/09; firmado por la autora 14/09): cierres [x] con su commit para las filas que git muestra cerradas; sin cambio de versión por protocolo (iv). U-COB-A queda ABIERTA hasta el laudo de la autora sobre las 77 unidades y `cifras_vigentes.md`.
...
9. **El grafo evaluado se sella y no se corrige.** Toda evaluación (extrínseca o
   ...
   **Registro del 20/09/2026 (laudo 3 de U-PLAN-MT — nombres y capítulos):** en la prosa de la tesis los dos objetos se llaman **«el grafo evaluado»** y **«el grafo corregido»**; los nombres de release del ciclo B2.6 (KG-Reextraído-r1, r2, r3…) se mantienen en este plan y aparecen en la tesis en una tabla del capítulo 4. El grafo evaluado es el **escalado que se sella antes de B6.3**: se presenta en la sección 4.5 «El grafo resultante» y se mide en el capítulo 8. El grafo corregido es la **release posterior al informe de B6.3 y B4**, declarada como tal: se presenta en el capítulo de conclusiones como entregable, con la lista de correcciones y su origen. (Texto tomado del laudo del 20/09; la correspondencia con las secciones de `main.tex` queda pendiente de verificar cuando se sincronice.)
...
- [x] A0.2 (I, $0) **HECHA** — Atribución determinística de primer nivel: fase A (regla `regla_atribucion.md`, 4 clases con precedencia presente→consultada→vista, veredicto por traza de esa misma respuesta, abstención como columna cruzada, selftest 24/24) ratificada y sellada en `40603a9`; fase B sellada en `85d9fdb` (`salida/atribucion_fallas.{json,md}`, hallazgos H1–H7, replay 120/120 base + 191/191 §7, doble corrida byte-idéntica).
  - [ ] **Pendiente del circuito — hecho 4 de U-INV-MT (21/09/2026), lo corrige la autora:** `data/experiment/ev2_reporte/regla_atribucion.md:3` conserva el encabezado «Estado: **PROPUESTA A LAUDO**» aunque la regla está sellada en el commit `40603a9` (verificado: ese commit incorpora el archivo). Corregir el encabezado a «sellada, commit `40603a9`». Relacionado, sin corrección: el reporte de atribución lee las clases `alcanzabilidad` y `vista_no_consultada` juntas como «navegación» (`salida/atribucion_fallas.md:257`, `:271`), y la clase `generacion` puede ocultar casos de nodo-ancla sin el contenido pedido, declarados como limitación de la clase (`:305-315`; `regla_atribucion.md` §7) — ver P-6.
...
- [~] B2.1 (I, $0) **FASE 1 (diagnóstico) CERRADA 15/09** (despacho `c6714a4`; inventario `inventario_B21_fase1.md`, reconstrucción del estado del backlog y tres sondas determinísticas en el paquete `revision_UB21_diag/`, fuera del repo; revisión de la mesa aprobada: manifest 15/15, estado del backlog y las dos sondas byte-idénticos al recomputarlos, tabla resumen recomputada contra el inventario). Resultado: 46 ítems = 12 BKL cerrados (estado efectivo `verificado`: 0003/0004/0005/0006/0007/0017/0019/0023/0026/0027/0028/0029) + 12 preguntas RT (no existen ids `RT-` en el backlog: son RT-C5-1..5, RT-C6-1..4 y RT-C7-1..3 de las propuestas y del retest C7) + T1–T7 e I1–I5 + 10 reglas de E4; 14 convertibles, 28 con condición, 4 no convertibles (BKL-0026/0027 son conducta del agente; I1/I2 exigen los grafos pre-merge), 15 dependen de retriever. Casi todas las condiciones son de una familia: adaptador de provenance gen 2/gen 3, catálogo parametrizado y queries que no están en el repo. Contradicciones registradas: C4 (`subclase_de` laudada en KG-Refinado) contra T7 de r1 (`padre_sugerido` flaggeado); BKL-0024/0025 nombrados «cerrados» en esta fila y `triaged` en el backlog. **FASE 2 (implementación) — decisiones de la autora (15/09):** (1) direccionamiento por (archivo del TO, punto ∈ conjunto de provenances, tipo, cadena normalizada), nunca `provenance[0]`; (2) adaptador de provenance gen 2/gen 3 dentro de la suite, sin compartir código con el perfil de B2.2; (3) política de cuarentena como parámetro por generación (`subclase_de` laudada en gen 2, `padre_sugerido` en gen 3), el laudo (d) de U-B1a decide el final; (4) tests de rank solo con el retriever in-memory que selló los ranks, config declarada por test, y la forma «posición en `ver_vecinos`» reducida a arista presente; (5) las queries RT de C5 y C6 ausentes del repo no se re-derivan: esas filas entran solo como tests de valor; (6) la salida es la tabla defecto → resuelto / persiste / no aplicable por grafo, con JSON; (6bis) el estado esperado por grafo se sella como fixture de la suite y la regresión se computa contra esa fixture, no contra la corrida anterior; (7) catálogo como parámetro leído del artefacto (`esquema_v2_clases.json` / `esquema_v3_clases.json`), nunca copiado. Mandato de fase 2 redactado como BORRADOR — PENDIENTE DE FIRMA DE LA AUTORA; no despachado. `scripts/regression_kg.py`: convierte cada BKL cerrado (C1–C7, BKL-0024/0025, RT-*), los tests T1–T7 de r1 **y las reglas declaradas de E4** (resolución de propuestos por label exacto / alias declarado / slug / singularización por token / sigla condicionada; TextoOrdenado solo desde provenance; materia/version como variantes) en **tests de respuesta conocida determinísticos** ejecutables sobre cualquier `kg.json` (nodo presente / ancla / valor / arista / rank en `buscar_nodos`). Salida: tabla defecto → resuelto/persiste — **entregable transferido explícitamente desde el issue #9 (cerrado en U-C0.1) a esta sub-tarea**.
  - [ ] **Pendientes del circuito surgidos del diagnóstico (no van a `backlog.jsonl`):** (1) **RX-10 PERSISTE en el vigente r1**: los montos del punto 1.2 de Capitales Mínimos están invertidos (Bancos `umbral` 2.500 / Restantes 5.000; sonda de anclas del paquete), el defecto que C2 corrigió a mano en KG-Refinado. El parser de B5.8.3 reconstruye la tabla testigo correctamente (`data/experiment/segmentacion_84/b583_tablas/reporte_b583.md:16`: pares por columna Bancos 5.000 / Restantes 2.500, pérdida 0,0; fila B5.8.3 de este plan), pero `e0_tablas.py` es un módulo standalone que solo consumen los scripts de censo de B5.8.3/B5.8.4 y de U-COB-A y su selftest: ningún módulo del pipeline de extracción (`e0_lib.py`, `runner_corpus.py`, `ensamblar_corpus.py`) lo invoca. La corrección «hacia adelante para r2» que esta fila enunciaba es NO ENCONTRADA como mecanismo: hay parser, no hay cableado. El test convertido de C2 dará «persiste» sobre r1 y sobre cualquier tanda hasta que ese cableado exista. (2) **BKL-0024/0025 resueltos por el pipeline sin cierre en el backlog**: T1 pasa sobre KG-Reextraído y r1 (21 nodos anclados en ext 3.9) y r1 tiene 5 nodos anclados en pro 1.1.1 con la definición de usuario; el backlog los deja `triaged` desde el 02/08. Cierre de la autora junto con la fase 2, declarando el cambio de verificación (de «re-corrida N=3» a test determinístico). (3) **B2.4 dice «15 BKL `triaged`» con un desglose que suma 16; el recomputo del estado efectivo da 17** (BKL-0001/0002 con `retriage_v3` sin `cambio_estado` posterior, 0008–0016, 0018, 0020–0022, 0024, 0025). Corrección de la fila pendiente.
- [~] B2.2 (I, $0) **FASE 1 (diagnóstico) CERRADA 15/09** (`5361ae7`: despacho asentado en esta fila; diagnóstico, censo determinístico y reportes `--out` de los dos validadores en el paquete `revision_UB22_diag/`, fuera del repo). Hallazgos que gobiernan la fase 2: los dos validadores (`scripts/shapes_validator.py` v0 y `grafo_v2/code/shapes_v2.py`) presuponen el formato de provenance `{source_doc, location}` que la generación 3 no emite (S4–S6 fallan 24.301/24.301 por construcción) y ninguno codifica el esquema congelado (9 / 13 / 26 pares, recontados desde `prompt_congelado.py`); referencias nodo→nodo (5.645) y `padre_sugerido` (41) son FAIL falsos en S1/S3; las 138 se reproducen (144 − 6); v0 rechaza en S1 las `miembro_de` que S15 exige; S15 falla sobre todo esqueleto v2. **FASE 2 (implementación) — decisiones de la autora (15/09):** opción A, extender v0 detrás de un perfil con default byte-idéntico (prueba: `run_3` contra `reports/shapes_run_3_v0.md`), con cláusula de escape: si exige reescribir el núcleo de v0, el ejecutor frena y lo declara como desvío; vocabulario leído de `prompt_congelado.py` (sha `e69feaaa…`), no copiado; tolerante con referencia nodo→nodo y `padre_sugerido` vía `rol_fuente`, sin exigirlos; severidad: bloqueantes S1–S6, catálogo, S15 y residuo del enum = 0, informativas S7/S8/S10/S11/S12 con conteo contra la línea de base de r1; `aplica_a` hacia cuarentena informativo. Mandato de fase 2 redactado como BORRADOR — PENDIENTE DE FIRMA DE LA AUTORA; no despachado. `scripts/shapes_validator.py` a esquema v2 (unidad 2 del tablero, pendiente desde julio): shapes de dominio/rango, provenance obligatoria, sujeto en catálogo o cuarentena. **Desde U-B1a**: extender `DOMAIN_RANGE`/shapes para `referencia` nodo→nodo y `padre_sugerido` (hoy fuera del esquema; las aristas llevan `rol_fuente`), y **todo test o shape que lea «el punto de un nodo» debe usar el conjunto de provenances, no `provenance[0]`** (138 aristas `referencia` cuyo destino exacto ≠ `provenance[0]` del nodo destino por fusión).
...
- [~] ESQ-1 (I, **tope USD 9,00 laudado** — presupuesto 6,38, cota alta 7,14, a re-presupuestar
  ...
  **Sub-unidades, secuencia fijada por el pre-registro `38be6e5` y corregida por `7072626`
  ...
  - [x] **U-ESQ-2-cal — Control del instrumento de descubrimiento (modo ii)** (cierre `eadf4a5`) (HECHA, sello
    ...
    real → C1.7 y ESQ-3.
  - [x] **Resolución del árbol: RAMA (b)** (cierre `f98005c`) — ESQ-2 protocolizada es la única vía; no hay censo.
...
- [x] ESQ-2 (H+I) **Test de cobertura del esquema, PROTOCOLIZADO — COMPLETA** (cierre `a7788c1`, lectura `b2e9e90`) (cierre del
...
- [x] ESQ-3 (H+I) **Gate: lectura conjunta + retoques + laudo de esquema congelado — CERRADO** (cierre `2593d4d`, pase `a7e426e`)
  ...
  **tabla de límites conocidos con tasas** (laudo §3). Exigencia 10 RESUELTA: schema-based
  ...
  - [x] **ESQ-3b — verificación pre-congelado, DOS VUELTAS** (cierre `100f5d7`) (costo total 0,2471 + 0,1567 =
...
- [x] ESQ-RI-1 (I, $0) **Modo de lectura sin raíz de sección** (cierre `b7654c8`) — **SE EJECUTA COMO B5.8.1** (adenda B5.5 firmada 06/09; fila sin duplicar): la causa proximal del cero es la compuerta de rol de página de E0 (`e0_lib.py:206-207`: sin página de índice previa, todas las páginas quedan en `portada`), y 47 de 53 TOs están en esa condición. Desbloquea **44 de 53**. **REFINADO por el censo B5.8.0 (06/09): el desbloqueo pleno vía B5.8.1 es 31 del RI — 11 TOs del RI sin espina de puntos utilizable pasan a candidatos a NO segmentable (se declaran en B5.8.4, no se fuerzan); el 47/53 pre-B5.2 concilia exacto con la medición vigente (42a+4b_idx+1c).** Es lo primero porque sin esto ninguna otra pieza puede siquiera medirse sobre el bloque. Caso aparte a tratar con remedio propio: 1 TO que sí llega a cuerpo y produce cero por no tener ninguna línea con formato de sección.
...
y antes de escalar).
- [x] B5.1 (I, $0) (cierre `bcb936a`) A1: parametrizar runner/E2/ensamblado por manifiesto (hoy cableado a 5 TOs; `censo_oraculo[to]` → KeyError; `LIMITACIONES_E0` hardcodeado; `ROL_POR_TO` con 5 keys); modo E2 sin oráculo. **Gancho de infraestructura (anotado, NO es compromiso de construirlo)**: la parametrización debe **contemplar la posibilidad** de construir un índice de fragmentos sobre el corpus escalado —el insumo que requeriría un eventual brazo comparativo en B6.3 (ver su regla de admisibilidad)—, porque dejar el gancho ahora es mucho más barato que agregarlo después. Si el brazo no se corre, el gancho queda sin usar y no cuesta nada.
- [x] B5.2 (I, $0) (cierre `16b0dc1`) A3: regex de E0 (`Sección N[.:]`, `Índice` sin guiones con guarda) → paridad 5/5 byte a byte + selftest 57/57 obligatorios; health-check por TO (`(cid:NN)`, páginas sin Sección). **CORRECCIÓN medida (U-B5.6-0, `docs/fe_erratas_D10_causa_regimen_informativo.md` §b): B5.2 NO desbloquea el bloque de régimen informativo.** En esos documentos la palabra que el regex busca no aparece —no hay marcador que relajar— y en un caso su única aparición es prosa sobre «índices de actualización», que un regex más laxo tomaría como marcador falso. B5.2 se mantiene por lo que fue escrita (paridad sobre normativa general y health-check); lo que desbloquea el bloque es el modo de lectura sin raíz de sección (ESQ-RI-1). **DESVÍO ACEPTADO del laudo de coordinación con U-B5.1 (registro para sesiones futuras — la instrucción vieja NO rige)**: el ítem `TO_KEYS` de U-B5.1 se resolvió SIN editar `e0_lib.py` (el driver itera el manifiesto), la cola interna de rebase quedó VACÍA, y al cierre de U-B5.2 corresponde únicamente **re-correr `selftest_manifiesto.py` completo** (P2 depende de la conducta de `e0_lib`) — no hay rebase pendiente.
- [x] B5.3 (I, $0) A4/A5 **CERRADA (05/09, USD 0)** (cierre `0515fa6`): reintento por corte de `max_tokens` (base 8.192 → una re-llamada a 32.768, request nueva con clave propia, ambos intentos persistidos íntegros; camino sin corte byte-idéntico — `selftest_manifiesto` 35/35 re-corrido por la instancia del plan, 0 misses proyectados 1.763/1.763, huella golden 14/14); guardia de cierre de fase (no cierra con reintentables pendientes); `PresupuestoCompartido` entre los 3 clientes (freno duro pre-llamada, estado persistido temp+rename); sub-chunking por ítems para unidades sobre umbral C8=26.182 estricto (dev intacto por construcción — el peor terminal dev mide exactamente el umbral; 6/6.670 anómalas en los 152, 0 en digeribles; unidad sin ítems detectables no se parte y queda declarada en `sub_chunking.json`). Selftest propio `selftest_ub53.py` 40/40; E0 57/57 y B5.2 39/39 intactos. Interpretación «16k→32k» declarada por regla d (ratchet E3 intacto; ver cola entrada 1). Residuo declarado: corte durante re-extracción del ratchet sigue a `cola_humana` (pase futuro posible). FAIL preexistente de `selftest_canal_abierto` (check de aditividad stale post-P1″, canal retirado) y efecto colateral de `selftest_corpus` sobre `salida_selftest/`: derivados al pase de higiene (cola entrada 14). **Con este cierre queda satisfecha la condición operativa del gate del escalado (laudo B5.5 §6, entrada 1).** Fe de erratas del despacho: `docs/fe_erratas_despacho_UB53.md`.
- [x] B5.4 (H+I) **CERRADA 06/09 (USD 0,3414 de tope 0,50; laudos `dea56ba` + `docs/laudo_B5.4_cierre_catalogo.md` firmados)**: catálogo de sujetos v3 sellado — prefijo sha `35e88c2dd0a2…`, hash/namespace `54a111e2175f`, **102 ids** (62 clases + 5 instancias + 35 roles; opción A2: 30 roles + 36 mapeos a clase + docvig/fimipyme sin rol; 5 retiros con lápida; adiciones SNP/BC-exterior/FMI/CEC; 24+6 definiciones dirigidas). **Verificación pareada 34 unidades + adjudicación de la autora (6 v3 / 1 antes / 2 otro): brazo B 8/8 y CERO cross-TO global (el mecanismo A2 resolvió la atracción de roles), PSTV resuelta por primera vez en tres iteraciones, anti-atracción del BCRA funcionando; mini-ciclo del laudo de cierre: def F1.3 PASA 3/3, rename y guarda con fallo declarado → residuos con vigilancias (7)(8)(9) en B6.1.** **U-CABLE-V3 CERRADA 06/09 (USD 0; diseño `docs/diseno_UCABLEV3.md` con las 3 resoluciones del freno 1)**: perfil de extracción por manifiesto (`perfil_e1.py`: `produccion_dev` default byte-idéntico / `v3_b54` con candado sha `35e88c2dd0a2…`/hash `54a111e2175f` que FRENA antes de toda llamada), vocabulario congelado COMPLETO en validador y E2 (verificado POR EXTENSIÓN: 102/102 + 9/9 + 13/13), ratchet con el perfil de su corrida, endurecimiento de `properties.tipo` solo-v3 (normaliza a «otra» con contadores — **instrumenta la vigilancia (5)**), C6/C7 versionados (manifiesto 35→37); batería completa re-corrida por la instancia del plan (cablev3 45/45, manifiesto 37/37 con 0 misses, ub53 40/40, congelado 46/46). El manifiesto del corpus escalado declara `perfil_e1: "v3_b54"` (nota en B5.7). Nota de integración cumplida: en los mapeos a clase el campo `rol_id` porta un id de clase. Texto original: **Catálogo de sujetos v3** congelado (SNP: entidad girada/depositaria/receptora/originante; bancos centrales, FMI, BIS, CCP; rol de alcance por TO nuevo) → rota el prefijo cacheado de E1 (aceptado). A2 de la auditoría. **CORRECCIÓN DE REGISTRO (07/09)**: esta fila decía «31 roles + 35 mapeos»; el artefacto sellado dice **30 + 36**. Causa: el mini-laudo del freno 2 de B5.4 movió `snp_cec` de rol a clase (31→30, 35→36) y la fila quedó con los números previos. Corregida por la instancia del plan; detectada por el relevamiento U-SUJ-ROL. **HERENCIA DEL LAUDO DE CONGELADO (`2593d4d` §4): B5.4 integra el prefijo congelado TAL CUAL (texto sha `e69feaaa…`, hash `1be8304e3d77`; el catálogo de sujetos es la única pieza que este laudo deja abierta) — cualquier diferencia con ese sha invalida la corrida que la contenga.** Herencias adicionales del laudo a este bloque: **regla de ensamblado — la unificación de nodos TO nunca se hace por rótulo** (riesgo de fusión medido en las lecturas); **remisiones intra-texto como criterio de aceptación de E3** (11/38 azarosas en ESQ-2); **endurecimiento del validador de `properties.tipo`** (hoy texto libre: `registro_contable` fuera de enum en producción).
- [x] **U-ESQ-V3 (I, $0) CERRADA 07/09** (cierre `b01eb18`) — cierra el hueco de cableado del esqueleto v3 (diseño en `docs/diseno_hueco_esqueleto_v3.md`; opción A laudada). **QUÉ QUEDÓ**: `data/experiment/esq_v3_miembros/esquema_v3_clases.json` con 66 clases (las 65 del v2 sin una edición + la CEC, con `padre_inferido: true` y su fundamento escrito) y 35 roles — los 5 dev con sus **17 miembros byte-idénticos** al artefacto sellado, verificado por el propio generador, que FRENA si alguno no pasa — más **34 aristas `miembro_de`** nuevas y **12 roles huérfanos** con su causa y su remedio: `sin_id_en_catalogo` 6 (autenf, pfmipyme, pimf, repefe, retype, traval → abrir id, re-sello del prefijo), `aplanamiento_rechazado` 5 (adrei, pagjub, ratiofn, rdbcra, snp_atm → clase más específica), `instancia_rechazada` 1 (ordcom → cambiar la firma de `miembro_de`, laudo sobre el esquema congelado). **S15 IMPLEMENTADA** en `scripts/shapes_validator.py` con el enunciado laudado —rol con `miembro_de` no vacío **o** en la lista declarada, miembros que son clases del árbol, y **FAIL si la cuenta declarada no coincide con la medida**— y orden explícito `[S1..S12, S15]`; S13/S14/S16/S17 declaradas no implementadas. **PASS sobre el grafo vigente** (5 roles, 17 aristas, 0 huérfanos), y S1–S12 byte-idénticas antes/después (12/12, re-verificado por la instancia del plan contra la línea base de HEAD). Inyección **aditiva y condicional al perfil** `v3_b54` en `ensamblar_corpus.py`, con default byte-idéntico (`salida/kg.json` `8e2eadee…`) e import diferido. **MEDICIÓN DE GENERALIZACIÓN DEL CATÁLOGO** (entregable nombrado, regenerable con `code/generalizacion_catalogo.py`): **34 de 69 colectivos de alcance = 49,3 %** tienen id de clase veraz en un catálogo derivado de 5 TOs, medido sobre 30 TOs frescos; descarte 35 **partido en dos porque los remedios son opuestos**: **26 = ausencia de concepto (74,3 %, remedio mecánico: abrir ids)** contra **9 = granularidad equivocada (25,7 %; 7 aplanamiento + 2 instancia, remedio: modelado)**. Se conservan las tres cifras rotuladas (33/69 sin rescates · **34/69 ADOPTADA** · 35/69 contrafáctico), porque su diferencia **es** el registro de la adjudicación. **NO ES COMPARABLE** con el 96,9 % de cobertura del tramo 2 (`main.tex:679`): difieren en denominador, unidad, material y —el que muerde— en que el 96,9 % cuenta lo que el extractor **eligió emitir**, de modo que un colectivo sin id puede no llegar a producir relación y caer fuera de su denominador. Las dos cifras no van juntas sin ese párrafo. **RESIDUOS A BACKLOG**: BKL-0028 (tres ids con definición doméstica y alias extranjeros — `Sujeto_entidad_financiera`, `Sujeto_banco`, `Sujeto_entidad_cambiaria`) y BKL-0029 (colectivo «titulares de cuenta corriente en el BCRA» sin id). **DEFECTO DE HERRAMIENTA CERRADO**: `DEFAULT_OUT = None` en el validador — su default apuntaba a un reporte commiteado y lo pisaba en silencio; declarado sin arreglar desde el 02/08 (`C3_retest_2026-08-02.md`, ahora cerrado) y disparado **al menos cuatro veces** (dos documentadas en agosto, al menos dos el 07/09 por autorreporte de las dos instancias, con **autoría no determinable** porque el validador es determinístico). Ítem 14(c) de `cola_mejoras_diferidas.md` **PROMOVIDO**, y entrada 15 nueva.
- [x] B5.5 (H) **Laudo D5** (firmado `c0daef1`): corpus a escalar — **T1, NO RECORTABLE** desde la reunión del 26/08 (el escalado es el objeto central de la tesis, no un capítulo opcional: el laudo ya no decide *si* se escala sino *qué y en qué orden*). Recomendación: los 68 digeribles primero (2.009 pág., 6.340 unidades, ~USD 123); RI (53 TOs, 0 digeribles) como segunda vuelta si B5.6 lo habilita. **[LAUDO ESCRITO REQUERIDO — toca compromisos del PPF/alcance]**: no arranca sin laudo redactado y fechado por la autora. Prerrequisito adicional: laudo de esquema congelado (ESQ-3). Checklist del gate: [ ] **cola de mejoras diferidas revisada** (`docs/cola_mejoras_diferidas.md` — las entradas 1 y 6 deben estar resueltas o re-diferidas con destino explícito antes de escalar; la 8 define el contenido de r2). **ESTADO 04/09: laudo COMPLETO y pendiente de firma** — §5 incorporado con el insumo de la reunión (§5.1 validación por lectura de capítulo = gate del escalado; §5.2 exigencia 7 → U-JOB-ACT, exigencia 6 no tratada; §5.3 Rama B para el catálogo de sujetos, borrador de mesa; checklist §6 del laudo verificado: entrada 1 re-diferida a B5.3, entrada 6 resuelta por B5.1 `bcb936a`, entrada 8 constatada en cola 8/12). **FE DE ERRATAS 05/09 (`docs/fe_erratas_despacho_UB53.md`): el «despachada y en curso» del §6 firmado era falso — U-B5.3 se despachó recién el 05/09; la sustancia del ítem (re-diferida con destino explícito, gate intacto) no cambia.** **ADENDA 06/09 FIRMADA (`docs/adenda_laudo_B5.5_segmentacion_universal.md`): segmentación universal de los 152 antes del escalado (secuencia B5.8, diseño aprobado); el corpus de EXTRACCIÓN de tandas 1–2 sigue en 68 salvo re-laudo con el censo de B5.8.0.**
- [x] B5.6 (I, $0) (cierre `d4e4e0a`, vía B5.8.3) Módulo de tablas (pdfplumber `extract_tables` con provenance, sin LLM) — RX-10 y montos invertidos. **SE EJECUTA COMO B5.8.3** (adenda B5.5 firmada 06/09; fila sin duplicar). **CORRECCIÓN medida (U-B5.6-0, fe de erratas §c): NO decide el destino del bloque de régimen informativo.** El parser alcanza a 23 de 53 TOs y al 12,4 % de las palabras del bloque: decide el de UNA de sus cinco familias estructurales. **Se construye igual y por sí solo**, porque RX-10 es un defecto de correctitud sobre **normativa general** —dos montos invertidos verificados— y ese defecto vive en el grafo que la tesis entrega. Su prioridad ya no depende de lo que se decida sobre el bloque.
- [x] **B5.9 — AMPLIACIÓN DE COBERTURA a los bloques excluidos (A y B) — DISEÑO APROBADO** (adenda 2 firmada y diseño aprobado `1ae387e`) (laudo de la autora del 07/09; adenda 2 al laudo B5.5 `docs/adenda2_laudo_B5.5_cobertura.md`, FIRMADA (1ae387e); diseño `docs/diseno_B5.9_ampliacion_cobertura.md`). Motivo (evidencia de U-COB-EXCL/-2): el instrumento deóntico no contaba el futuro impersonal — corregido, la relación material-excluido/articulado cae de 5,58× a 1,56×; lectura a ciegas de 32 páginas con pre-registro sellado (23 prescriptivo / 9 referencia / 0 duda); **diez de los doce no segmentables contienen contenido prescriptivo** y solo plandecuentas y optico son referencia pura (densidad deóntica 0,000). **El criterio de forma no coincide con el de contenido: el recurso excluiría normativa vigente.** Alcance recomputado contra `particion_152.json`: bloque A = **10 TOs / 41 páginas** para extracción (enmienda 1 de la autora: `optico` 43 pág y `plandecuentas` 77 pág quedan DECLARADOS referencia pura —densidad deóntica 0,000 en las tres variantes, ya adjudicados por la lectura a ciegas— y se censan sin extraer; control 161−43−77 = 41 recomputado); bloque B = ficha de los 2 parciales / **2.175 páginas** (manual 1.830 + ri2_pm 345). Hallazgo del recomputo: hay **2.281** páginas `ficha_registro` en total — las otras 106 (plandecuentas 76, ri_laft 27, ri_transpa 3) se tratan por herencia al cierre de B, no antes.
  - [ ] **U-COB-A** (10 TOs, 41 pág: ri_con 16, ri_spi 11, ri_tii 6, ri_rem 2 + seis de 1): A.1 diagnóstico + modelo de unidad/procedencia + encaje en el esquema + censo de los dos declarados referencia ($0, FRENO) → A.2 piloto pago → A.3 masa. Proyección: 41 × 1,38 = ~**57 unidades ≈ USD 1,10**. **Procedencia LAUDADA**: bloque de prosa en página, sin fabricar espina; `documento + página + offset`; campo `granularidad_procedencia` (`punto` / `pagina`) por elemento; **el reporte da el conteo EXACTO de elementos con granularidad de página sobre el total del recurso** (para escribir «N de M, todos de los diez documentos sin numeración»). **A.1 CERRADA con sus hallazgos y CUATRO LAUDOS de la autora (07/09)**: (1) la mitad del bloque A NO es prosa — 21 de 41 páginas son planilla/ficha (185 de 332 bloques; solo 147 útiles de prosa+mixta), lo que **enmienda la caracterización** de la adenda («prosa sin numeración») sin tocar su decisión; la escala **NO se lauda por la muestra de UNA página de planilla dentro del alcance** (`ri_con` p.16, juzgada referencia) sino que **se manda al piloto A.2 con DOS BRAZOS declarados —prosa y planilla— y predicciones pre-registradas por separado**; (2) **`ri_spi` SALE del bloque A** (numera el 71 % de sus bloques: darle procedencia de página degradaría granularidad real, y darle la suya pide código que A.1 tiene prohibido escribir) y se declara caso propio —tiene espina, el parser vigente no la reconoce; destino regla de marcador o unidad chica—, con **alcance final de NUEVE documentos** y páginas/bloques recomputados por el ejecutor; (3) la tesis escribe **la fila en unidades** (medición), recomputada tras (2) y el piloto, y la fila en elementos del grafo se marca EN EL TEXTO como **proyección NO verificada** (regla i ampliada); (4) piloto A.2 aprobado con **tope USD 2,00**, composición re-dimensionada, en freno exprés antes de gastar. **Guarda 1 verde en los cuatro controles** (1.763 con desglose, 762, 6.340, 9.266) y cero módulos tocados; verificador propio 29/29 corrido también por la instancia del plan desde cwd ajeno. **Correcciones de conteo asentadas**: las 12 unidades degeneradas NO son «una por documento» (plandecuentas 0, ri_pspii 2) — el bloque A reemplaza **11**, imprecisión del mandato de la instancia del plan; y la muestra de planilla dentro del alcance es de **una** página, no tres — imprecisión de la revisión, corregida por la autora. **Cruce prescriptividad × forma (registro §6 de la adenda)**: las 10 prescriptivas del bloque A son TODAS prosa, con correspondencia 1:1 por documento con «diez de los doce»; y **13 de las 18 fichas del bloque B fueron juzgadas prescriptivas**, lo que deja asentado que el desacople es de **calendario y no de fondo** — así lo declara el capítulo. **TRES DESTINOS, NO DOS (registro §7 de la adenda)**: con `ri_spi` afuera, los diez documentos con prescripción se atienden por tres vías — bloque A (nueve, ahora) · `ri_spi` (caso propio, variante de marcador o unidad chica) · bloque B (release posterior) — **y el capítulo debe declararlo así o parecerá que un documento se perdió en el camino**. Efecto medido de la salida de `ri_spi`: son **141 de 332 bloques = 42,5 %**; el alcance queda en **9 documentos / 30 páginas / 191 bloques** (77 prosa + 114 planilla), la categoría mixta desaparece (sus 37 bloques eran todos suyos), el brazo de prosa cae a la mitad y **la planilla pasa a ser mayoría (60 %)**. De ahí el agregado de la autora al freno exprés: **si un brazo queda sin material suficiente para medir, es hallazgo con su cuenta — no se compensa ni se sigue con un brazo simbólico.**
  - [ ] **U-COB-B** (ficha, 2.175 pág): B.1 diagnóstico con **volumen medido**, **adjudicación de contenido a ciegas espejo de la del bloque A** (el bloque B no tiene esa evidencia) y **encaje en el esquema congelado** ($0, FRENO) → B.2 piloto ri2_pm → B.3 manual con laudo posterior. Sin ancla de densidad para material ficha: ~3.000 unidades ≈ USD 58 a densidad de prosa, potencialmente un múltiplo. **Umbral de balance: >25 % de las unidades del recurso ⇒ no arranca sin laudo.**
  **SECUENCIA (respuesta explícita del diseño):** el capítulo del esquema y la lectura de mentores NO se detienen — el tramo 5 narra la partición de B5.8.4 como hecho Y declara el hallazgo de cobertura con la ampliación en curso (HECHO: §3.3 `main.tex:571`, `ed389c4`). **La fecha del ESCALADO sí se mueve: 7–13 sesiones, del orden de 2–3 semanas de trabajo efectivo** más adjudicaciones. **DECISIÓN DE SECUENCIA LAUDADA (07/09): DESACOPLE** — A antes del escalado, B como **release posterior declarada** (principio 9); el «ambos, no uno» sigue en pie en el qué (B se declara con su evidencia y su B.1 corre cuando el escalado esté encaminado). **Efecto en la fecha del escalado: días, no semanas.** El tramo 5 declara el hallazgo de cobertura con la ampliación en dos tiempos. `main.tex` NO se toca por esto: procedencia y partición se ajustan en el tramo 5.
- [ ] B5.7 (I) Issue #6: documento de costos con tarifas reales + caching + experimento óptimo dentro de USD 200 (con B5.5). Laudo D4 warm-then-parallel (throughput: ~13 s/unidad → ~29 h secuenciales para 8.010). **Incluye (re-laudo 06/09): re-presupuesto explícito de los no-RI reconocidos por B5.8 antes de que la tanda 2 los gaste** (base informativa: tabla de los 31, cota superior ~USD 69,73). **El manifiesto del corpus escalado que esta unidad construye declara `perfil_e1: "v3_b54"`** (U-CABLE-V3 cerrada: el campo existe, el candado del sello corre al cargarlo).
- [ ] **B5.8 — SEGMENTACIÓN UNIVERSAL del corpus (los 84 «necesita reglas») — DISEÑO APROBADO 06/09; adenda al laudo B5.5 FIRMADA 06/09** (`docs/adenda_laudo_B5.5_segmentacion_universal.md`; diseño completo `docs/diseno_B5.8_segmentacion_universal.md`; resoluciones del §7 en la firma de la adenda: corpus de extracción sigue en 68 con re-laudo post-censo; segmentable ≠ extraíble ratificado; ESQ-RI-1 y B5.6 se ejecutan como B5.8.1/B5.8.3).
  - [x] **B5.8.0 — censo de familias (CERRADO 06/09, USD 0, revisiones coincidentes)** (cierre `ece8607`): 84 = 53 RI + 31 no-RI clasificados con evidencia por TO (`data/experiment/segmentacion_84/censo_84.{md,json}`, determinismo verificado por regeneración byte-idéntica); familias primarias a=44 · b_idx=7 · b_sec=1 · b_pts=22 (17 no-RI: enganchan y fallan umbrales C4–C8; 5 marginales adjudicables en B5.8.4 sin regla) · c=9 · e=1; veredictos preliminares **70 segmentables / 2 parciales (cuerpo ficha) / 12 candidatos a NO segmentable**; conciliación 47/53 pre-B5.2 exacta (42a+4b_idx+1c); cruces con lo sellado exactos (23/53 tablas, 12,4 % = 69.543/562.622, 62 con cero unidades). Hallazgo de proceso: la sonda del scoping no matcheaba «INDICE» en mayúsculas — matiza «la palabra no está» para 6 TOs, reportado sin ajustar sellados. **RE-SECUENCIACIÓN asentada: B5.8.3 rinde recién TRAS B5.8.1** (0 TOs con tabla como falla primaria; los 6 con tabularidad ≥40 % están detrás de la compuerta). **RE-LAUDO DE LA AUTORA (adenda §2.1, 06/09, opción i): la tanda 2 incorpora EN PRINCIPIO los no-RI que B5.8 reconozca, con corte final post-B5.8.4 sobre unidades reales, health-check en verde por TO como condición de entrada por documento, y re-presupuesto explícito en B5.7 antes de gastar** (estimación informativa de la tabla de los 31: ~3.586 unidades ≈ USD 69,73, COTA SUPERIOR extrapolada — ningún número extrapolado se compromete). El RI sigue segmentable-no-extraíble (adenda §2.2).
  - [x] **B5.8.1 — modo de lectura sin raíz de sección (CERRADA 06/09, USD 0; ejecuta ESQ-RI-1; cierre `b7654c8`; revisiones mesa + plan coincidentes, ambas con verificación independiente)**: los 41 TOs objetivo del censo (recomputo regla d: 41, no 42 — `ri_acsf` remite a B5.8.3 y corrió como control negativo declarado, que se comportó como debía) **rinden 41/41 con 923 unidades nuevas**, activación por cero unidades 42/42 (garantía estructural: el modo solo corre donde el camino vigente produjo cero — cableada y verificada), health-check por TO 30 sanos / 11 con señales declaradas (5 `cid` de fuente preexistente, 7 con sub-chunking de B5.3 interviniendo; 8 particiones, 0 no-particionables), cuerpos-ficha de manual/ri2_pm declarados fuera vía rol `ficha_registro` (umbral 0,30 calibrado), guardas G0–G3 ancladas a casos medidos + contraejemplos (banner ≥3 páginas, forma de título, monotonía con `salto_raiz`, columna), preámbulo sintético S0 con cobertura de cero pérdida. **Batería de byte-identidad COMPLETA verificada además por re-corrida propia de la instancia del plan**: dev 20/20 vs salida_enm01, 68/68 vs e0_dry (8 artefactos por TO, difs vacía), 10 ESQ 10/10, conteos 1.763/6.340/762 exactos, selftests 35/35 + 57/57 + 39/39 + 40/40 + b581 34/34 (desde cwd ajeno). Débiles declarados sin forzar (ri_tar, ri_transpa: reinicio de numeración por apartado — límite medido, territorio B5.8.2/B5.8.4); hallazgo del banner de página registrado (límite en TOs ≤2 páginas declarado). Escrituras: e0_lib.py + healthcheck_e0.py + selftest_b581.py + `segmentacion_84/b581_sin_raiz/`. Tras esta sub-unidad la partición proyectada queda ~111 reconocidos de 152, con B5.8.2 (marcadores, chica) y B5.8.3 (tablas) como colas independientes que gatean B5.8.4.
  - [x] **B5.8.2 — reglas de marcador por familia (CERRADA 06/09, USD 0; cierre `b45d341`; revisiones coincidentes: mesa por consistencia y tallies, instancia del plan por corrida propia completa)**: objetivo recomputado por regla d contra el censo = **7 TOs** (6 variantes de índice + reqcac), no los «7+1» del mandato — **imprecisión DEL MANDATO registrada como propia de la instancia del plan** (no descontó a `ri_tsa`, que el censo ya marcaba enganchado por B5.2); `ri_tsa` corrió como control declarado (modo `vigente`, 15 unidades, jamás entró a las ramas nuevas). **RINDEN 7/7 con 317 unidades** (consyr 32, nmaeef 39, reqcac 3, ri_dcpc 129 con gramática plena de puntos, ri_msrl 11, ri_psp 5, seguef 98), activación por cero 7/7, salud 4 sanos + 3 con señales declaradas. Reglas R1 (variantes de índice: «INDICE»/«ÍNDICE»/«Índice -»/«– Índice –», guarda de tres capas) · R2 (encabezados de sección CAPS y con letra, número verbatim, sucesión por familia de interpretación con `salto_seccion` reportado — errata IV→VI de ri_psp) · R3 (banner de caja mixta ≥3 páginas — la causa de las 274 huérfanas selladas de reqcac), todas detrás de `marcadores_b582=False` por default y ancladas a casos medidos + contraejemplos («3.7.2. Indice a utilizar» rechazado; la continuación de índice NO extendida por el contraejemplo de ri_dcpc p.3). Límites DECLARADOS sin regla, a B5.8.4: reinicio por anexo de nmaeef (mismo patrón ri_tar/ri_transpa), afinados b_pts. **Batería VERDE verificada además por corrida propia de la instancia del plan: dev 20/20, 68/68, 10 ESQ, conteos 1.763/6.340/762, y PROTECCIÓN de B5.8.1 re-corrida con el código nuevo — 42/42 byte-idénticos**; selftests E0 57/57 · B5.2 39/39 · b581 34/34 · b582 nuevo 59/59 · ub53 40/40 · manifiesto 37/37 (post-U-CABLE-V3, atribución verificada por diff). Escrituras: e0_lib.py (+237/−17), healthcheck_e0.py, selftest_b582.py, `segmentacion_84/b582_marcadores/`. Con este cierre solo falta B5.8.3 (tablas) para habilitar B5.8.4 (partición final).
  - [x] **B5.8.3 — parser de tablas (CERRADA 06/09, USD 0; ejecuta B5.6; cierre `d4e4e0a`; revisiones coincidentes: mesa por consistencia y tallies, plan por verificación propia)**: módulo **standalone** `e0_tablas.py` (solo stdlib+pdfplumber; cero ediciones a módulos vigentes — garantía de no-cambio ESTRUCTURAL: los caminos sellados ejecutan el mismo código) + `selftest_b583.py` 33/33 + `segmentacion_84/b583_tablas/`. Objetivo recomputado: 30 TOs (27 familia d del censo ∪ 4 con `ficha_registro` de b581, ∩ ri_laft). **RINDEN 29/30: 563 tablas lógicas = 559 parseadas + 4 declaradas con causa** (pérdida real de reconstrucción 22–55 %, no emitidas), 643 segmentos, 80 costuras (30 candidatas declaradas sin fusionar), 9.481 filas, 39 notas al pie; ri2_pm 0 parseadas DECLARADO (material ficha, consistente con censo); ri_acsf (control negativo de b581) rinde por su vía: 1 tabla lógica de 2 segmentos. **CASO TESTIGO RX-10 RESUELTO**: `capmin::tabla000` p.4 con pares por columna {Bancos: 5.000, Restantes: 2.500} y pérdida 0 — el par que la linealización invirtió en el grafo dev (remedio de raíz hacia adelante; el grafo sellado se corrige en r2). Reglas con anclas y contraejemplos (R-TC/R-COL/R-COSTURA/R-ENC/R-VERIF por multiconjunto/R-NOTA); los 3 hallazgos de calibración convertidos en reglas con contraejemplos. **Contradicción del mandato resuelta por regla d — SEGUNDA IMPRECISIÓN DEL MANDATO registrada como propia de la instancia del plan** (asignó los cuerpos-ficha de manual/ri2_pm que el censo ya mandaba declarar en B5.8.4; el parser confirmó con medición propia que su vía no es tabular). Batería completa VERDE (dev/68/ESQ/b581 42/42/b582 8/8 + selftests, manifiesto 37/37). **B5.8.4 queda gateada solo por este cierre.** Secuencia de 5 sub-unidades, todas USD 0 de API, segmentación mecánica sin LLM: B5.8.0 censo de familias de formato de los 84 (53 RI + 31 no-RI) → B5.8.1 modo de lectura sin raíz de sección (**ejecuta ESQ-RI-1**) → B5.8.2 reglas de marcador por familia (patrón B5.2: ancladas a casos medidos + contraejemplos) → B5.8.3 parser de tablas (**ejecuta B5.6**) → B5.8.4 partición final 152/152 + health-check por TO reconocido + no-segmentables DECLARADOS con causa + reporte recomputable (insumo de las filas 10–15 del mapa de fuentes del capítulo). **Regresión de no-cambio DURA en cada sub-unidad**: byte-identidad E0 de dev/68/ESQ (1.763 / 6.340 / 762) + selftest_manifiesto 35/35 + E0 57/57 + B5.2 39/39 + ub53 40/40; cualquier diferencia = FRENO. **Frontera conceptual declarada: segmentable ≠ extraíble** — el RI sigue fuera de extracción hasta su ciclo propio (D10); el corpus de extracción de tandas 1–2 sigue en 68 salvo RE-LAUDO con el censo en la mano (adenda §2.1). Corre en PARALELO a U-CAP-ESQ; **B5.8.4 es prerrequisito del CIERRE del capítulo** (tramo 5 incorpora los números nuevos), no de su escritura.
  - [x] **B5.8.4 — PARTICIÓN FINAL (CERRADA 06/09, USD 0; cierre `78a757c`, corrección post-freno `07d29e4`; revisiones coincidentes: mesa por sumas, instancia del plan por recomputo completo de los comandos publicados + resolución de las dos preguntas de la autora). CON ESTE CIERRE, B5.8 COMPLETA CIERRA y la adenda B5.5 §1 queda CUMPLIDA: la partición del corpus es un HECHO verificado.** Partición 152 = **138 reconocidos plenos (4.183 pág / 9.266 unidades / 548 tablas) + 2 parciales declarados (manual, ri2_pm: cuerpos-ficha → familia «parser de registro» declarada) + 12 no segmentables declarados con causa** — 6.757 pág / 9.324 unidades / 559 tablas, todo con comandos de recómputo (insumo de las filas 10–15 del mapa del capítulo). Adjudicaciones contra criterios sellados citados por fila: 22 b_pts (fallas C* remanentes DECLARADAS, afinado no construido), ri_tsa con límite (26/61 páginas pre-índice rinden por vía tabular), reinicios por anexo/apartado con límite, 12 candidatos NO confirmados (incl. los 3 degenerados ri_chr/ri_pspii/ri_rem — 1–2 unidades con raíz, declarados por criterio censal, ratificado en la firma). **Diferencias censo↔corrida explicadas con causa**: rmgcti 32→81 unidades — **B5.2 lo curó entero** (evaluar sellado: digerible, cero fallas; rótulo de la fila corregido en el cierre); ri_rml/snp_dd pierden C8 por sub-chunking de B5.3 (artefactos presentes) y manori/cateloc/snp_mep lo retienen (no particionables declarados); ri_cc suma C6 por instrumento más fino (46/118 pág con tabla real = 39 % > 20,24 %). «Rinden 142» del log = contador operacional (138 − ri_acsf tabular + 2 parciales + 3 degenerados), distinto de la partición adjudicada — manda la partición. Hueco fimipyme S1 PERSISTE (anunciado_sin_cuerpo medido → vigilancia (6) de B6.1); docvig con entrada de alcance en índice + unidad 2.4. **Tabla del re-laudo de tanda 2: 30 no-RI, 2.007 unidades REALES, 12 con salud sana estricta** — la definición operativa de «verde» (sano estricto vs señales-declaradas-aceptables) queda para el re-laudo con B5.7. **Tramo 5 del capítulo habilitado en cuanto los tramos 2–4 existan.**

### B6 · Escalado: corrida por tandas (issue #11) — **T1** (ruta crítica desde la reunión del 26/08; era T2) · S6–S7 (máquina; humano mínimo)
- [ ] B6.1 (I, ~$40) Tanda 1: 20 TOs digeribles (normativa general prioritaria), E0–E5, **gate de release de B2.6** (regression + shapes + intrínsecas + material propio) antes de ensamblar, carga en Neo4j, app sirviendo el grafo. Reporte: volúmenes, costo real vs estimado, incidencias. Es la primera ejecución del método de B2.8 de punta a punta sobre TOs nunca vistos. **VENTANA DE CORRECCIÓN DE ESQUEMA (laudo `2593d4d` §7)**: la tanda 1 es el test de generalización del esquema congelado; su reporte MIDE los cinco ítems de vigilancia pre-declarados — (1) migraciones tipo-Condicion contra la guarda de modalidad, (2) conteo de vaciamientos vs tasas del laudo §3, (3) duplicación de contenido entre cajas, (4) rótulos de TO (unificación solo por identidad documental), (5) emisiones residuales de vocabulario retirado (debe ser 0) — **INSTRUMENTADA por U-CABLE-V3: contadores del validador en modo v3 (normalización de `properties.tipo` a «otra» + contador separado de `requisito_de_estructura`), fluyen al resumen de corrida** — con muestreo a definir en su mandato; **(6) — agregada por laudo B5.4-f1 (`docs/laudo_B5.4_fase1_catalogo.md`): unidades propias vacías — secciones del índice del PDF sin unidad E0 (caso testigo: la sección «Entidades alcanzadas» de fimipyme), medidas con el health-check de B5.2 (`healthcheck_e0.py`); (7)–(9) — agregadas por el laudo de cierre de U-B5.4 (`docs/laudo_B5.4_cierre_catalogo.md` §5–§6), con tasas y muestreo a sellar en el mandato de B6.1: (7) roles de alcance en `ejecuta` sin apoyo textual (tasa; si es mala, candidata a cuarentena determinística en el validador, con laudo en la ventana), (8) tasa de `sujeto_propuesto` contra la base 3,1 % con umbral pre-declarado (válvula cerrándose de más), (9) emisiones de `entidad_originante_de_transferencia` fuera del dominio de pagos (muestreo; si la tasa es mala, retiro a r2 con caso de promoción)** — una clase nueva de falla DE ESQUEMA admite UN ciclo de corrección con laudo, siempre antes del pre-registro de B6.3.
  - [ ] **CHECKLIST DEL GATE — Esqueleto v3 inyectado y S15 en verde** (asentado por U-ESQ-V3, commit `b01eb18`). El `kg.json` de la tanda lleva las aristas `miembro_de` de los roles de alcance v3, inyectadas desde `data/experiment/esq_v3_miembros/esquema_v3_clases.json` por el paso de esqueleto que `ensamblar_corpus.py` invoca **cuando el manifiesto declara** `perfil_e1: "v3_b54"` — nunca por la mera presencia del archivo. **VERIFICACIÓN**: `python3 scripts/shapes_validator.py --kg <kg.json> --excepciones data/experiment/esq_v3_miembros/esquema_v3_clases.json --out <dir de la tanda>/gate/shapes_tanda1_<fecha>.md`, con **S15 en PASS**. El `--out` va **explícito, fechado y único por tanda, hacia una ruta VERSIONADA del propio release**, y la razón se escribe acá para que nadie lo simplifique de vuelta: **el reporte del gate es la evidencia de que la verificación corrió, así que tiene que quedar archivado con la tanda que aprueba** — no en una ruta temporal, que no sobrevive. Lo que el defecto de `b01eb18` prohíbe no es escribir en el repo: es escribir **por default, en silencio y colisionando entre corridas**. Su default era `reports/shapes_run_3_v0.md`, un reporte del registro que cualquier corrida pisaba sin aviso; hoy el default es `None`, de modo que **omitir `--out` no rompe nada pero tampoco deja reporte que archivar**, que para un gate es igual de inservible. Historia del defecto en `data/backlog/retests/C3_retest_2026-08-02.md`. **QUÉ EXIGE S15**: todo rol tiene `miembro_de` no vacío **o** figura en la lista declarada de roles sin miembro adjudicable; sus miembros son clases del árbol; y **la cuenta declarada coincide con la medida** — si un rol deja de ser huérfano y nadie actualiza la lista, S15 FALLA. **Sin `--excepciones` la lista es vacía y todo huérfano es FAIL**: es el default seguro, no un atajo. **SI S15 FALLA, EL RELEASE NO SALE**: un rol huérfano significa que las normas de ese TO no son alcanzables por navegación taxonómica, que es el mecanismo que el capítulo del esquema presenta como aporte del grafo frente al fragmento. **La lista de excepciones no es hipotética: son DOCE roles con su causa y su remedio** — `sin_id_en_catalogo` (6: autenf, pfmipyme, pimf, repefe, retype, traval → abrir id, re-sello del prefijo v3, unidad propia; BKL-0028/0029) · `aplanamiento_rechazado` (5: adrei, pagjub, ratiofn, rdbcra, snp_atm → una clase más específica en el catálogo) · `instancia_rechazada` (1: ordcom → cambiar la firma de `miembro_de`, laudo de la autora sobre el esquema congelado).
  - [ ] **CHECKLIST DEL GATE — Retiro de las 3 aristas `aplica_a` adjudicadas como falsedad** (asentado por U-COB-A). Las 77 unidades de prosa del bloque A entran al recurso **menos** estas tres, que la adjudicación de U-COB-A listó para retiro y que **ninguna unidad retiró** —la unidad lista, el ingreso es acto del ensamblado—: (1) `Operacion «Transferencias entre cuentas CVU mismo PSPCP»` → `Sujeto_pspcp` y (2) la misma `Operacion` → `Sujeto_entidad_financiera`, ambas de **`ri_tii::p5.b9`**, unidad clasificada **(B)**: es una definición de código de partida («2010100 - De CVU a CVU dentro del mismo CBU recaudador») con `aplica_a` colgado encima, gemela de diez casos ya adjudicados como no-destinatarios; (3) `Operacion «Presentación de informaciones al BCRA»` → `Sujeto_pscpp`, de **`ri_pscpp::p1.b0`** — **arista (B) dentro de una unidad (C)**: la unidad tiene destinatario legítimo y conserva sus otras 4 aristas, pero ésta se construyó sobre el **título de la norma remitida** y no sobre el pasaje. **POR QUÉ ES BLOQUEANTE**: una `aplica_a` sobre algo que no obliga a nadie es **falsedad en campo estructurado**, el único caso que el principio §1 del laudo del esquema congelado (`2593d4d`) manda **RETIRAR** y no aceptar con residuo — y es invisible justamente porque valida contra el esquema. **CONDICIÓN DE CIERRE VERIFICABLE**: el `kg.json` de la tanda **no contiene ninguna de las tres**; se comprueba buscando las dos `Operacion` nombradas y sus destinos en `edges`. **Si alguna está, el release no sale.** Procedencia: `data/experiment/cobertura_bloque_a/recomputo_concordantes.json` §`b_aristas_que_se_retiran` y `revision_concordantes.json`.
  - [ ] **CHECKLIST DEL GATE — La tanda 1 pasa por r1** (asentado 15/09 desde el diagnóstico de U-B2.2 fase 1): §1.2 de la tesis compromete remisiones resueltas sobre el grafo ensamblado; la tanda 1 pasa por r1; queda por fijar la mecánica. Evidencia del hueco: los pasos de r1 (E4 determinístico, `padre_sugerido`, referencias nodo→nodo, provenance rica) viven en `ensamblar_r1.py` y sus módulos `r1_*`, y `ensamblar_corpus.py` (el ensamblador que este ítem y el anterior nombran) no los importa; NO ENCONTRADO en el repo un artefacto que los una. No es una decisión abierta: es mecánica a fijar en el mandato de esta unidad.
- [ ] B6.2 (I, ~$85) Tanda 2: resto de digeribles (48) si tanda 1 cierra sin sorpresas. Créditos AWS/Bedrock si aplica (`app/llm_backend.py` ya soporta Bedrock). **RE-LAUDO 06/09 (adenda B5.5 §2.1, opción i): incorpora EN PRINCIPIO los TOs no-RI que B5.8 reconozca — corte final post-B5.8.4 sobre unidades reales, health-check en verde por TO como condición de entrada por documento, re-presupuesto en B5.7 antes de gastar.**
- [ ] B6.3 (H+I, ~$25–40 **a estimar**) **EVALUACIÓN FINAL DE LA TESIS sobre el grafo escalado** — rediseñada por el principio 10 (era «sanity funcional descriptivo»). Es la medición del **conjunto de test**: se corre **una sola vez**, con **pre-registro sellado propio** (mismo molde que EV2 / A1.3: hipótesis, umbrales, criterios de lectura y tope de costo declarados ANTES de mirar resultado alguno). Componentes: (a) **eval set fresco sobre el corpus amplio** —preguntas nuevas con gold por criterios, selladas antes de correr, construidas sobre TOs **disjuntos del subset de desarrollo (los cinco TOs) Y de los 10 documentos de ESQ-1/ESQ-2 (misma lista, corrección D4 del laudo `8dea823`)** listados en `data/experiment/esq/documentos_excluidos_esq.json` (creado en `a7788c1`, 10 IDs con sha256 verificados contra el manifest), porque el esquema final se retocó mirándolos (ver nota de alcance del bloque ESQ); jamás EV2, que es material de desarrollo y está quemado. La disjunción se verifica contra ese archivo y se declara en el reporte—; (b) fidelidad con el juez calibrado + adjudicación humana simétrica; (c) atribución causal determinística A0.2; (d) **evaluación intrínseca por tripletas** con el instrumento ya validado sobre r1 (D-f): precisión, importancia y recall del gran grafo. **Sin brazo comparativo por defecto** (D-h: la tesis se enuncia como trabajo de recurso; la comparación contra recuperación por fragmentos es validación de diseño sobre el conjunto de desarrollo, no la pregunta central de la evaluación final) [SUPERADO por D-h2, 20/09/2026: el brazo de fragmentos entra a B6.3, componente (f)]. **Regla de admisibilidad de una eventual comparación sobre el grafo escalado — A DECIDIR EN EL MOMENTO DEL PRE-REGISTRO DE B6.3, no antes** [DECIDIDA por D-h2, 20/09/2026: el brazo se declara en el pre-registro sellado, con peso confirmatorio]: si se la quiere con **peso confirmatorio**, debe estar **declarada en el pre-registro sellado** de B6.3 (brazo, material, métrica y umbrales, antes de mirar nada); si se corre **después** de conocido el resultado de B6.3, se reporta explícitamente como **análisis exploratorio posterior** y **nunca al mismo nivel** que la evaluación confirmatoria. Regla dura: si el resultado obliga a tocar el pipeline, el arreglo produce una **release posterior** y se declara como tal (principio 9) — no se re-corre esta evaluación para mejorar el número. **Componentes agregados por laudos del 20/09/2026:** (e) **exactitud de cita** — tres indicadores determinísticos por respuesta, sin juez ni API: cita existente (toda cita parseable resuelve a un punto del índice E0 de su TO), cita fundada (toda cita es fiel a alguna provenance vista en la traza) y cita al punto de referencia (alguna cita cae en el ancla del gold o debajo de ella); valores `sí` / `no` / `sin_citas` / `sin_json`; fracción cruda «n de N» por tanda, sin umbral, sin pool entre tandas y sin cruce con veredictos ni atribuciones; instrumento validado en desarrollo por U-CITA-2 (`scripts/ucita2_indicadores.py`, `reports/ucita2_indicadores.{json,md}`, JSON sha256 `a9d32d3b…`; sobre r1 base, contenido N=31: existente 31, fundada 30, al ancla 28; tres re-corridas con control de 0 discrepancias). La redacción previa de esta fila (e) que el laudo 5 menciona como ya redactada por esa instancia es NO ENCONTRADA en el repo; esta redacción se escribió desde los artefactos. (f) **brazo de fragmentos (D-h2)** — el sistema por fragmentos definido por A2.1 (fragmentos de E0 con herencia, top-k=5, mismo agente con una sola herramienta de recuperación), configuración principal BM25 **y configuración secundaria denso (`harrier-oss-v1-0.6b`), las dos en B6.3 con el mismo protocolo, juez, criterios y preguntas que (b)** [LAUDO 6 DEL 20/09/2026, reemplaza el «NO DECIDIDO» anterior]; declarado en el pre-registro antes de cualquier corrida. **Válvula de costo (laudo 6):** si el costo medido de c_agente en A2.2 supera en más del doble la estimación de esta fila, el pre-registro de B6.3 puede reducir la secundaria a N=1 sin re-corridas; esa reducción se declara en el pre-registro y no después. **Costo estimado del brazo — ESTIMACIÓN NO VERIFICADA:** el plan no tiene una fórmula escrita para el brazo del grafo (esta fila dice «~$25–40 a estimar»; fórmula NO ENCONTRADA), así que se escribe una con las bases que el plan sí tiene: juez de fidelidad USD 7,29 por 40 preguntas sobre r1 = USD 0,18 por pregunta (B1.8); agente del banco USD 0,534 por 12 sesiones de la fase B = USD 0,045 por sesión (A2.0-banco; sesiones cortas de smoke, no representativas de una corrida completa); A2.2 presupuesta ~USD 15 para tres brazos con N=1 y N=3 parciales sobre 40 preguntas. Fórmula: costo_brazo = N × (c_agente + c_juez) × (1 + f_N3), con N = tamaño del eval set fresco (a fijar en el pre-registro), c_juez ≈ 0,18, c_agente a medir en A2.2 antes de sellar B6.3, f_N3 = fracción de preguntas re-corridas con N=3. Con N=40, c_agente ≤ 0,20 y f_N3 ≤ 0,5: ≈ USD 15–23 por configuración (ESTIMACIÓN NO VERIFICADA); la configuración secundaria entra (laudo 6) y duplica ese monto, salvo la válvula de N=1.
  - [x] **U-EV2-TIPO** (H, $0) **— CERRADA 21/09/2026 (commits `537d8da`, `493875d`, `ee1169a`; ver CIERRE al final de esta fila)** — **clasificación por tipo de las 40 preguntas del conjunto EV2**, abierta por el laudo 4 del 20/09/2026, prerrequisito del sellado de B6.3. La autora clasifica las 40 preguntas antes de sellar B6.3, con una **regla escrita antes de leer ninguna** (entregable previo a la lectura): cuántos puntos hacen falta para responder según el ancla y los criterios (uno / más de uno) y si la respuesta correcta es que la norma no lo prevé. La clasificación se guarda en un archivo aparte junto a `data/experiment/exploracion/ev2_fidelidad/preguntas_ev2_fidelidad.json`, **sin modificar el JSON sellado** (sha256 `1d587336…`; al 20/09 sus preguntas solo llevan `id`, `to`, `to_nombre`, `pregunta`, `gold{ancla, criterios}` — sin campo de tipo). La misma regla se aplica al conjunto de preguntas nuevas del test final (B6.3 (a)) cuando se construya. Cubre la promesa P-3 del §6. **U-EV2-TIPO-0 — regla SELLADA el 20/09/2026** en `data/experiment/exploracion/ev2_fidelidad/regla_tipo_pregunta.md`, sha256 de sellado `1b20aa18979b02b60c2412ebf470808122adc1c162259f980fc24f9ae847f0f9`, commit `5278f93`. Verificación del 20/09: el archivo commiteado en `5278f93` (= HEAD) tiene sha256 `35c8be10…` porque ya incluye la sección «Enmienda 1 — 20/09/2026, antes de leer ninguna pregunta»; el sha de sellado `1b20aa18…` es exactamente el del contenido del archivo hasta antes de esa sección (recomputado por truncamiento), es decir, la regla sin la enmienda. Los dos sha se registran para que el sello sea recuperable. **Continúa con U-EV2-TIPO-1**: enmienda 1, previa a la lectura, que fija que el ancla comprende sus subpuntos, más la hoja de lectura; después el control con modelo y el cierre. **ESTADO 20/09/2026 (actualización de la autora):** U-EV2-TIPO-1 CORRIDA el 20/09 — hoja de lectura `data/experiment/exploracion/ev2_fidelidad/tipo_pregunta_hoja.csv`, sha256 `30c1c02892e9ef68ff664b0728dd149cc96185280815d654d72c025689a199ed`, 40 filas, las 40 anclas resueltas en E0 con su subárbol completo, sin NO ENCONTRADO (verificado el 20/09: sha, 40 filas con columnas `orden, id, to, pregunta, ancla, criterios, texto_ancla, tipo, nota`, 0 celdas con NO ENCONTRADO; el archivo está sin trackear al momento del asiento). U-EV2-TIPO-1b DESPACHADA (confirmado por la autora): genera el documento de lectura para la autora y corre el control del modelo bajo la misma regla, con su salida sellada hasta que la autora termine su clasificación. **Pendientes:** [x] clasificación de la autora; [x] comparación con el control del modelo; [x] cierre con `tipo_pregunta_ev2.json`. **CIERRE (hechos verificados el 21/09 contra los artefactos y los commits; los tres pendientes anteriores quedan cerrados):** (1) regla `regla_tipo_pregunta.md` sellada con la Enmienda 1, sha256 `35c8be10…`, commit `5278f93`; la regla v1 no se modifica. (2) Hoja `tipo_pregunta_hoja.csv` (40 preguntas en orden aleatorio, semilla 20260920; sha256 `30c1c028…`), documento de lectura `tipo_pregunta_lectura.md` (sha256 `4c70ad73…`) y control del modelo `tipo_pregunta_control.json` (`claude-sonnet-4-6`, temperatura 0, sin thinking, 40 llamadas, 0 reintentos, 0 sin respuesta; sha256 `b5cf3b26…`), los tres en el commit `537d8da`, anterior a la clasificación de la autora; costo USD 0,29 calculado por la ejecutora de TIPO-1b a partir de los tokens informados por el proveedor (78.516 de entrada, 3.578 de salida, precios de lista de Sonnet 4.6); el JSON no lleva campo de costo, NO VERIFICADO contra artefacto. (3) Clasificación de la autora `tipo_pregunta_autora.csv`, a ciegas respecto del control, sha256 `44700810…`, commit `493875d`: **40 dato directo, 0 varios puntos, 0 abstención, 0 dudoso**. (4) Comparación: modelo 38 dato directo y 2 varios puntos; acuerdo simple **38 de 40**; desacuerdos EV2F-014 (ext:13.5) y EV2F-005 (ext:4.6.1), adjudicados por la autora como dato directo en `tipo_pregunta_adjudicacion.csv` (sha256 `84a59882…`); tipo final **40 dato directo** en `tipo_pregunta_ev2.json` (sha256 `f96055e0…`, `resumen.distribucion_tipo_final`); scripts `scripts/ev2_tipo_generar_hoja.py`, `ev2_tipo_generar_lectura.py`, `ev2_tipo_control.py`, `ev2_tipo_comparar.py`; base de captura de las llamadas `control_tipo_ev2.db` (sha256 `2c226238…`); todo en el commit `ee1169a`. Todos los archivos en `data/experiment/exploracion/ev2_fidelidad/`. **(5) Enmienda PENDIENTE para una versión 2 de la regla** (interpretación asentada en la adjudicación, que no modifica la regla v1): un criterio que pide una remisión tal como el ancla la enuncia no exige el contenido remitido, y que el ancla remita a otro punto no cambia el tipo si ningún criterio pide ese contenido. La regla v2 debe sellarse **antes de clasificar cualquier pregunta del conjunto final, y no antes**. **Consecuencias sobre B6.3 (a) asentadas en la fila del requisito de diseño de B6.3 (a).**
  - [ ] **Requisito de diseño de B6.3 (a) — consecuencia declarada de U-EV2-TIPO (20/09/2026), PENDIENTE de que la autora fije las cuotas; no es decisión tomada.** El conjunto EV2 se generó con una unidad por pregunta y con criterios tomados de esa misma unidad (`data/experiment/exploracion/ev2_fidelidad/registro_generacion_ev2_fidelidad.md` §4, pasos 1–3: una pregunta por unidad seleccionada, con 2–5 criterios verificables y cita textual de la unidad) y no contiene preguntas cuya respuesta correcta sea que la norma no lo prevé (las 40 llevan `gold.ancla` y `gold.criterios`, verificado el 20/09 sobre el JSON `1d587336…`); clasificación hecha (U-EV2-TIPO, cerrada 21/09): 40 de 40 dato directo, 0 varios puntos, 0 abstención. (c) el requisito de diseño de B6.3 (a) queda CONFIRMADO CON EVIDENCIA: EV2 tiene 40 preguntas de dato directo y ninguna de varios puntos ni de abstención, así que el conjunto final debe incluir esos dos tipos por diseño; la cifra de cada cuota es **decisión ABIERTA de la autora, que la consultará con los mentores** — sin números propuestos. (d) La clasificación del conjunto final se hará con el mismo procedimiento — clasificación de la autora a ciegas más control del modelo — con la regla v2, como parte de B6.3 (a). El conjunto de preguntas nuevas del test final debe construirse incluyendo, **por diseño y con cuota declarada en el pre-registro**, preguntas de varios puntos y preguntas de abstención, porque sin ellas la comparación del componente (f) no puede mostrar la capacidad de seguir remisiones que la introducción presenta como motivación. Cuotas: a fijar por la autora en el pre-registro de B6.3.
  - [x] **U-CITA-2 — CERRADA 18/09/2026** (registrada el 20/09 por laudo 5 de U-PLAN-MT): indicadores determinísticos de exactitud de cita sobre KG-Reextraído-r1, sin juez ni API. Artefactos: `scripts/ucita2_indicadores.py`, `reports/ucita2_indicadores.md` y `reports/ucita2_indicadores.json` (sha256 `a9d32d3b…`; verificado el 20/09). Tres indicadores por respuesta — cita existente, cita fundada, cita al punto de referencia — con valores `sí` / `no` / `sin_citas` / `sin_json`, sin umbral y sin agregación entre tandas; sobre r1 base, contenido N=31: existente 31, fundada 30, al ancla 28 (recomputado el 20/09 desde `agregados.ev2_r1_base.contenido` del JSON); tres re-corridas (enc_r1/r2/r3, N=19/18/19) con control de discrepancias del indicador 1 = 0 (`conciliacion.control_ind1_discrepancias_total`). Es validación en desarrollo del componente (e) de B6.3, no resultado de la tesis. Commit de cierre: `fb6ef69` (asentado el 20/09 por laudo de cierre de U-PLAN-MT; hash verificado, y los tres archivos entran al índice en ese commit).
...
- [ ] C1.1 **Introducción** — estructura en ocho ideas (cada una un párrafo o dos): (1) los LLM habilitaron la construcción de KGs a escala y por qué los KGs importan para RAG; (2) limitaciones actuales, **solo las que este trabajo resuelve** (fidelidad no medida, comparación sin baseline justo, castellano/banca central, costo, evaluación intrínseca ciega a la sobre-fusión); (3) «acá se resuelve X, Y, Z»; (4) overview del método (corpus → esquema → E0–E5 → retrieval → evaluación bajo custodia → refinamiento por releases); (5) el grafo en sí (esquema, sujetos, provenance) con **una figura** de un subgrafo real; (6) la evaluación: «los grafos son difíciles de evaluar, por eso se desarrolló…» (juez de dos pasos, fidelidad por criterios, atribución causal, tripletas con importancia); (7) los números principales (EV2 3/20/17 · 5/26/9 · 4/27/9; ablación; head-to-head cuando exista); (8) lista de contribuciones. Pregunta + tres sub-preguntas + hallazgo rector quedan dentro de (3)/(6). **Primer borrador en S2**, antes de que haya más resultados: se actualiza después. (S2–S3)
  ...
  - [x] C1.1 **tramo de objetivos — DESBLOQUEADO por D-g** (cierre `2f511ba`, `69689e8`): el objetivo general se formula sobre el corpus completo (recurso final) y los específicos incluyen la validación del método sobre el conjunto de desarrollo.
  - [x] C1.1 **tramo de Estado del arte — DESBLOQUEADO** (cierre `2f511ba`, `69689e8`): el material de background llegó (ver U-RW y `docs/lecturas_reunion_2026-08-26.md`).
  - [x] **U-FIG-RAG · U-FIG-PROC-V · U-FIG-TRIPLETA-2 — CERRADAS, cierre común `952b78a`** (fila creada el 20/09/2026 por el laudo de cierre de U-PLAN-MT: NO EXISTÍA fila previa para estas unidades en el plan). Figuras 1.2, 1.3 y 2.1 con sus generadores y el reporte `reports/verificacion_figura_proceso.md` (`952b78a`: 8 archivos; el mapeo unidad → número de figura no se verificó contra `main.tex` en este pase). Archivos por unidad, según sus generadores: `figura_fragmentos_vs_grafo` (U-FIG-RAG), `figura_proceso_extraccion` (U-FIG-PROC-V), `figura_tripleta` (U-FIG-TRIPLETA-2). **U-FIG-TRIPLETA-2**: `docs/tesis/figuras/figura_tripleta.png` sha256 `f9e28792…`, `figura_tripleta.svg` `7cd77c05…`, `generar_figura_tripleta.py` `4be0079b…` (los tres verificados el 20/09), sobre la arista 16669 de `salida_r1/kg.json` `0226e947…` (`generar_figura_tripleta.py:62` y `:112`).
...
- [ ] C1.5 Metodología de evaluación bajo custodia (pre-registro, sets ciegos, juez 2 pasos, juez de fidelidad por criterios, verificador v7', taxonomía, métricas intrínsecas, regression suite, backend/retrieval) + **los dos instrumentos declarados** (principio 8): harness congelado para lo sellado, banco Claude Code + MCP para el head-to-head, con el puente A1.7 y el adaptador de trazas que preserva la atribución. (S3–S5)
  - [ ] **Vigilancia de la escritura — hecho 1 de U-INV-MT (21/09/2026): el tope de llamadas se controla por turno.** Las tres herramientas del agente son `buscar_nodos(consulta, limite)`, `ver_nodo(id)` y `ver_vecinos(id, direccion)` (esquemas en `data/experiment/evaluacion/harness.py:240-286`; implementaciones `:148`, `:179`, `:197`; verificado). `MAX_TOOL_CALLS = 15` (`:50`) se comprueba al cierre de cada turno (`:527`), después de procesar todos los bloques `tool_use` del turno, así que un turno con varias llamadas puede excederlo: en las **456 trazas de la corrida base de EV2** (`ev2_corrida/trazas/ev2_base_{run3,v3,v2}/`, dos ejes: 120 de fidelidad y 336 pares de navegabilidad; KG-Base 160, KG-Refinado 168, KG-Reextraído 128) **80 superaron 15 llamadas, máximo 18** (recomputado el 21/09 con el comando del inventario). Sobre otra población —las 430 trazas de fidelidad de base, §7 y r1— el recuento da 99 con máximo 17; el capítulo debe decir sobre qué trazas cuenta. **Corrección PENDIENTE en la tesis:** `main.tex` (copia sincronizada `a1e53b5`) dice en 2.3 «un tope de llamadas por pregunta, 15 en este trabajo, tras el cual debe responder con lo que reunió» (`:487`) y en 3.6 «con un máximo de 15 consultas por pregunta» (`:727`); ninguna de las dos refleja el control por turno (la cita del hecho 1 del mandato corresponde a 2.3, no a 3.6). **Compromiso:** el capítulo 5 describe el control por turno con la cifra 80 de 456 y máximo 18.
  - [ ] **Sección «El agente y el sistema de consulta» al inicio del capítulo 5 — propuesta por la mesa (U-INV-MT, 21/09/2026), pendiente de que la autora escriba su prosa.** Hecho 5 verificado sobre `main.tex` `a1e53b5`: ningún capítulo describe el sistema en prosa (herramientas, tope, abstención, índice, juez); las menciones son de paso (`:454`, `:487`, `:489`, `:727`). **Matiz sobre el mandato:** el encabezado `\section{El agente y el sistema de consulta}` (`sec:sistema`) ya existe en `main.tex:1194` como primera sección del capítulo 5, con el esquema 5.1.1–5.1.5 en comentarios (`:1196-1227`) y sin prosa; los capítulos 6 a 9 son solo encabezados (`:1229-1242`). Lo pendiente es la prosa, no la creación de la sección.
- [ ] C1.6 Resultados: 2.3; escalón 1/1b; C1–C7; intrínsecas; EV2 (fidelidad+navegabilidad+atribución); ablación retrieval; head-to-head; gold tripletas; escalado. (S5–S8)
  - [ ] **Vigilancia de la escritura — hecho 2 de U-INV-MT (21/09/2026): cada tabla de resultados de la tesis declara qué índice de texto usó.** Verificado: las mediciones de desarrollo de EV2 usaron el índice en memoria `GraphIndex` (intersección de tokens sin pesos ni stemming, `harness.py:148-162`; corrida base vía `FullCaptureAgent(GraphAgent)`, `ev2_corrida/code/runner_ev2.py:64`; r1 declarado in-memory en `ev2_r1/preregistro_ev2_r1.md:29`; re-corridas de §7: mismo harness, runner de encadenamiento no releído en este pase — NO VERIFICADO). El índice fulltext de Neo4j 5.26.9 (`data/experiment/neo4j/docker-compose.yml:34`; analizador spanish, campos `label`, `descripcion`, `description`, `id_texto`, similitud Lucene BM25 por defecto — `neo4j_index.py:32-55`, `indices.py:12-19`; constantes de BM25 no verificadas) se usó en la ablación A1.4 (400 trazas), en el banco MCP (A2.0-banco, pieza (i)), en la app y en el escalado (`docs/laudo_promocion_backend.md` §3; `docs/decision_backend_grafo.md:11,122`). **Backend de la medición final:** el laudo A1.6 §3 fija que «Neo4j es el backend del corpus completo; el índice booleano in-memory no se usa a escala» para el escalado (B5/B6); no nombra a B6.3, pero B6.3 mide el grafo escalado, así que queda fijado por ese laudo y el pre-registro de B6.3 debe declararlo. No es decisión abierta.
...
- D-g **RESUELTA — laudo firmado 27/08/2026** (borrador redactado: `docs/laudo_D-g_alcance_corpus.md`):
  ...
  **Nombre del grafo escalado (laudo 3 del 20/09/2026, donde este laudo lo dejaba sin nombre):** en la prosa, **«el grafo evaluado»** (el escalado sellado antes de B6.3); su etiqueta de release en el ciclo B2.6 se asigna al sellarlo y no queda fijada por este laudo.
...
- D-h **RESUELTA — decisión de la autora, 27/08/2026**: ENCUADRE DEL ENUNCIADO DE LA TESIS. La
  ...
  **Consecuencias**: (a) B6.3 **no lleva brazo comparativo por defecto** [SUPERADA por D-h2, 20/09/2026]; (b) A2 (head-to-head)
...
- D-h2 **LAUDO DE LA AUTORA, 20/09/2026 — COMPARACIÓN EN EL TEST FINAL (modifica D-h).** La comparación contra recuperación por fragmentos deja de ser solo validación de diseño sobre el conjunto de desarrollo y pasa a ser **también un componente de B6.3**, sobre el grafo escalado y con el mismo conjunto de preguntas del test final. El pre-registro de B6.3 incorpora el brazo de fragmentos **antes de cualquier corrida**, con las mismas reglas de sellado que el resto (hipótesis, umbrales, criterios de lectura, tope de costo). **Filas que cambian por este laudo:** B6.3 — la cláusula «Sin brazo comparativo por defecto» queda SUPERADA y la «Regla de admisibilidad … A DECIDIR EN EL MOMENTO DEL PRE-REGISTRO» queda DECIDIDA (el brazo entra al pre-registro con peso confirmatorio); B6.3 suma el componente (f) con su costo estimado; A2.1 — sus tres decisiones (fragmentos de E0 con herencia, top-k=5, mismo agente con una sola herramienta de recuperación) pasan a ser la definición canónica del sistema por fragmentos que B6.3 reusa; A2.2 y A2.3 — la enumeración de brazos anterior queda SUPERADA por el laudo 2 del 20/09 (BM25 principal, denso secundario, híbrido fuera de B6.3); D-h — consecuencia (a) SUPERADA, consecuencia (b) se mantiene (A2 sigue siendo validación de diseño sobre el subset y así se reporta); M-15 del §6 — la demostración suma B6.3 (f); P-2 del §6 — la promesa se cumple con dos columnas. **Riesgo declarado**: amplía el enunciado de D-h en la dirección del PPF; se menciona a los mentores con el pre-registro de B6.3.
- D-i **DECISIÓN ABIERTA de la autora (registrada 21/09/2026, hecho 5 de U-INV-MT): el objetivo específico 6 de la introducción no tiene sección que lo cubra.** El sexto de los seis objetivos específicos («Implementar y probar la actualización del recurso ante cambios en la normativa…», `main.tex:275` en `a1e53b5`): el capítulo 9 (`:1238`) cubre el objetivo 5 y ninguno cubre el 6. Opciones: (i) una sección propia en el capítulo 4, con el material de U-JOB-ACT (cerrada, `9278923`: primera corrida 157/157) como base; (ii) trabajo futuro con reformulación del objetivo. Sin recomendación de la mesa.
...
- [x] **U-CAP-ESQ** (`ed389c4`; capítulo escrito y enviado 11/09; la validación escrita de los mentores es el gate y sigue abierta en el changelog — registrado por instancia del plan 11/09; firmado por la autora 14/09) (H+I, escritura) — capítulo de esquema y validación pre-escalado en el
  ...
  **NOTA DE COORDINACIÓN (06/09, tuneo de la autora)**: el título de la sección pasó a
  «Diseño y validación del esquema del grafo» (label `cap:esquema` intacto); los mandatos de
  los tramos 2–5 referencian el título VIGENTE en `main.tex`, no el del esqueleto. El §1 sumó
  hechos nuevos en el tuneo (nombres de los 5 TOs, rango 40–204 páginas, composición 4+1) con
  filas de mapa en verificación por la mesa de tuneo. **B5.8.4 es prerrequisito del CIERRE del
  capítulo** (tramo 5 incorpora la partición nueva), no de su escritura.
  **FE DE ERRATAS del tramo 1 (defecto del CIRCUITO de redacción-revisión, no solo de la
  plantilla)**: el mandato del tramo 1 llevó la transversal «primera persona del singular»
  contra el preámbulo sellado del informe (`main.tex:35`: registro formal e impersonal), y la
  revisión de mesa tampoco lo detectó antes del despacho; el ejecutor lo resolvió por regla d
  (mandan los archivos) y lo reportó. Saldado en la plantilla de los tramos 2–5: el registro
  del informe lo fija su preámbulo; la primera persona rige los documentos de trabajo.
  - [~] **ESTADO DEL GATE (11/09)** — capítulo enviado 11/09, esperando validación escrita de
    los mentores. (registrado por instancia del plan 11/09; firmado por la autora 14/09). Tramos escritos y commiteados: §3.1–§3.10 y §4.1/§4.2/§4.6 en
    `ed389c4` (tramo 2 `bf4a22c`; tramo 3 `d09219d`, `eafdb04`; capítulo completo `ed389c4`).
    Placeholders que quedan en `main.tex@ed389c4`: §4.3 `:1047`, §4.4 `:1053`, §4.5 `:1059`
    (alimentados por la tanda 1, que este gate bloquea). Las tres decisiones abiertas y los dos
    ítems (tabla §3 del laudo vs Tabla 4; «reemplaza 11» de `:647`) están en la entrada de
    estado del 11/09 del changelog.
- [x] **U-JOB-ACT** (cierre `9278923`) (I, $0 diseño) — job recurrente mensual de actualización: re-scraping
  ...
  **CERRADA 07/09 (fases 1 y 2, USD 0; revisiones coincidentes). Exigencia 7 CUMPLIDA con
  ...
  No se deriva de acá ninguna tasa de churn del corpus.** **DEUDA ANOTADA (declarada por el
  ejecutor al cerrar): el §2.a.6 del diseño commiteado ancla la periodicidad con «8 de 152
  (5,3 %)» y «3 de 5 (60 %)» — escritos ANTES de que la regla de conteo se fijara, y hoy la
  contradicen. El artefacto está commiteado y la unidad cerrada, así que no se retoca: la
  deuda es que NINGÚN tramo del capítulo ni registro futuro cite esos porcentajes desde esa
  sección; se cita la fracción cruda con su ventana al lado, que dice lo mismo sin sugerir
  una tasa. Si un tramo futuro toca esa sección por otro motivo, los corrige de paso.** Dos hallazgos metodológicos
...
- [x] **U-EXP5** (I, $0) **FASE DE INVESTIGACIÓN CERRADA 15/09** (despacho `3246e51`; entregable `evaluacion_por_etapas_UEXP5.md` en el paquete `revision_UEXP5/`, fuera del repo; revisión de la mesa aprobada: recomputo de conteos byte-idéntico, anclas muestreadas correctas, sin recomendaciones al pipeline ni al esquema). Resultado: 23 enfoques con precedente, siete etapas con sus instrumentos y estado [existe y reporta / existe sin tasa / no existe], 18 filas de costo marcadas ESTIMACIÓN NO VERIFICADA. Hallazgo rector: la única tasa del repo atribuida a una etapa concreta es la del censo de anclas de EV2 (7/40 no resueltas por granularidad de E0 en r1); el resto por etapa son booleanos o conteos sin publicar; la desagregación por etapa de una precisión medida sobre tripletas es un cruce en código vía la provenance de r1 (`chunk_id`, `estado_e3`, `rol_fuente`), como ya anticipan la exigencia 5 del mapa y los requisitos de B4.1. La propuesta a los mentores la decide la autora; destino B4.1. — investigación de metodología de evaluación del pipeline POR ETAPAS
  - [ ] **Destino del entregable (decisión de la autora, 15/09):** se versiona tras la verificación de los externos por la mesa (sin fecha); entran `evaluacion_por_etapas_UEXP5.md` y `lista_externos_UEXP5.md`, congelados tal como los entregó la ejecutora (sha256 del manifest), más el script `recomputo_conteos_UEXP5.py` y su salida, porque el documento afirma que sus conteos se recomputan con ese script y por la regla i el ancla tiene que estar en el repo; más una nota de verificación de la mesa como archivo aparte. No entran los textos extraídos de PDF de terceros ni el archivo del grep de convenciones.
  - [ ] **Pendientes del circuito surgidos del diagnóstico (no van a `backlog.jsonl`):** (1) `calibradores_e3.py:37-38` afirma «detección gold 8/8» con puntero a `resumen_faseB_e3.json`, y ese archivo no tiene ningún campo `gold` — comentario sin artefacto que lo respalde; (2) E3 verificó 1.757 unidades contra 1.763 del corpus de desarrollo (`estado_corpus.json`), diferencia no investigada; (3) las métricas intrínsecas M1–M10 nunca corrieron sobre KG-Reextraído ni r1 (rutas fijas en `scripts/metricas_intrinsecas.py:64-66`) — es B3.2, sigue abierta.
...
freno **(fe de erratas 05/09: recién despachada ese día — `docs/fe_erratas_despacho_UB53.md`)**;
la demo para la reunión con el contacto es la app sobre el grafo vigente (r1), no
...
elegibles. Incorporado como §5.1 del laudo B5.5 (firmado, `c0daef1`). Punto CERRADO.
...
| M-15 | Comparación justa KG-RAG vs RAG sobre los mismos chunks de E0 | A2 head-to-head + Pareto, sobre el banco de M-21 (mismo agente y mismo prompt en los dos brazos); **desde D-h2 (20/09/2026) también B6.3 (f) sobre el grafo escalado**, con el sistema por fragmentos definido en A2.1 (BM25 principal, denso secundario; las dos configuraciones corren en B6.3 por laudo 6 del 20/09/2026, con válvula de N=1 para la secundaria) | — (depende de A2.0-gate y A2.0-banco) | **pendiente** |
...
| P-1 | **PROMESA** de 2.1.4 al capítulo 8: muestra de hechos juzgados contra su pasaje y estimación de cobertura | B4.1 (vía de precisión: 100 tripletas con evidencia, correctitud + importancia, adjudicación de a 10, juez calibrado; vía de recall: tripleta más importante por fragmento, ranking por importancia, orden de las top-100 validado por lectura humana, presencia por regla, adjudicación post hoc solo de ausentes) → B4.2, B4.3, B4.4; validado sobre KG-Reextraído-r1 y medido en B6.3 componente (d) (laudo D-f) | texto tomado del mandato U-PLAN-MT del 20/09; pendiente de verificar contra main.tex cuando se sincronice | **PROMESA** — la hace 2.1.4; la debe el capítulo 8 |
| P-2 | **PROMESA** de 2.2.1 al capítulo 7: las tres decisiones del sistema por fragmentos (fragmentación, medida de afinidad, cuántos fragmentos y en qué orden) declaradas antes de correr; los dos sistemas juzgados con el mismo juez y los mismos criterios | A2.1 (pre-registro sellado: fragmentación = chunks de E0 con herencia; afinidad = BM25 en el baseline y denso `harrier-oss-v1-0.6b` en el brazo RAG, laudo A2.0b; top-k=5 por ranking del recuperador; mismo agente del banco A2.0 en los dos brazos con una sola tool por brazo; mismo juez de fidelidad EV2 y mismas 40 preguntas) → A2.2, A2.3; encuadre D-h: validación de diseño sobre el conjunto de desarrollo | texto tomado del mandato U-PLAN-MT del 20/09; pendiente de verificar contra main.tex cuando se sincronice | **PROMESA** — la hace 2.2.1; la debe el capítulo 7. **Laudo 2 del 20/09/2026:** la promesa se cumple con **dos columnas**, BM25 (principal) y denso (secundaria), del mismo sistema por fragmentos definido en A2.1; el híbrido B1.9 no la cumple ni entra a B6.3. **Laudo 6:** las dos columnas se reportan también en el capítulo 8 (B6.3 (f)), con la secundaria eventualmente a N=1 si el pre-registro lo declara |
| P-3 | **PROMESA** de 2.2.2 al capítulo 5: el conjunto de evaluación distingue preguntas de dato directo de preguntas de varios saltos; los tres indicadores de cita (existente, fundada, al punto de referencia) se reportan | Conjunto EV2 `data/experiment/exploracion/ev2_fidelidad/preguntas_ev2_fidelidad.json` (sha256 `1d587336…`, 40 preguntas con claves `id`, `to`, `to_nombre`, `pregunta`, `gold{ancla, criterios}`): **clasificación hecha el 21/09 (`tipo_pregunta_ev2.json`, sha256 `f96055e0…`): 40 de 40 dato directo; la decisión de cuotas para el conjunto final sigue pendiente de la autora, que la consultará con los mentores**. Indicadores de cita: NO ENCONTRADO en este plan al 20/09 (ninguna fila los nombra) [SUPERADO el mismo 20/09 por el laudo 5: U-CITA-2 CERRADA 18/09, registrada bajo B6.3, y componente (e) de B6.3] | texto tomado del mandato U-PLAN-MT del 20/09; pendiente de verificar contra main.tex cuando se sincronice | **PROMESA** — la hace 2.2.2; la debe el capítulo 5. **Laudos 4 y 5 del 20/09/2026:** la clasificación de preguntas queda cubierta por **U-EV2-TIPO** (sub-ítem de B6.3, regla escrita antes de leer, archivo aparte del JSON sellado) y los tres indicadores de cita por **U-CITA-2** (cerrada 18/09) y el componente (e) de B6.3 |
| P-4 | **PROMESA** de 2.2.2 al capítulo 6: las cuatro clases de atribución, en este orden, ausencia en el grafo, no alcanzado, visto sin consultar, consultado y mal respondido | A0.2 (regla sellada `data/experiment/ev2_reporte/regla_atribucion.md:152-159`: clases `ausencia_kg`, `alcanzabilidad`, `vista_no_consultada`, `generacion`; el orden de la promesa es el embudo de lectura de `:161-167`, que la regla declara equivalente a su precedencia presente→consultada→vista); M-19 | texto tomado del mandato U-PLAN-MT del 20/09; pendiente de verificar contra main.tex cuando se sincronice | **PROMESA** — la hace 2.2.2; la debe el capítulo 6 |
| P-5 | **PROMESA** (registrada 21/09/2026, hecho 3 de U-INV-MT): el capítulo 5 reporta las tres validaciones del juez de EV2 con sus cifras y distingue ese juez del de la sección 3.6; el capítulo 2 solo conserva 11 de 12 y 52 de 53 | Juez de EV2: `claude-sonnet-4-6`, temperatura 0 (`ev2_juez/juez.py:38`), prompt sha256 `fd446f8e…` (`ev2_reporte/reporte_ev2.md:286`), unidad (respuesta, criterio), N=3 y voto modal, ciego al grafo (`ev2_juez/README.md:14-19`; mapping `ev2_juez/mapping.py:40-54`). (1) Calibración U6: 25 respuestas y 92 criterios, 14 acuerdos / 6 desacuerdos / 5 a adjudicación; v1.1 con aceptación no cumplida, 3 cambios de veredicto modal en 91 pares (`ev2_juez/calibracion/registro_calibracion.md:21,45,100-115`). (2) Muestra simétrica ciega: 12 fichas, acuerdo exacto 11/12 y por criterio 52/53; único desacuerdo en dirección B, juez parcial-incorrecto y humana correcta, 1/9 (`reporte_ev2.md:290-296`). (3) Hoja de trabajo: 48 fichas / 200 criterios adjudicadas (`reporte_ev2.md:447`); «30 requiere_adjudicacion resueltos 20 y 17» NO VERIFICADO: el reporte registra 21 heredados resueltos 2/18/1 y 9 de §7 resueltos 1/5/3 (`:110-111`). Aparte, el juez de la sección 3.6 (`evaluacion/judge.py` v2.1.1, dos pasos, `evaluacion/02_calibracion_juez.md`): 12 trazas (6 preguntas × 2 corridas); «20 celdas, 12 de 12» NO VERIFICADO — la tabla tiene 26 celdas con veredicto humano, 25 acuerdos y 1 desacuerdo, y `main.tex:727` dice «12 de 12 casos de prueba»; «referencia humana no ciega» NO VERIFICADO (el archivo declara input ciego al grafo y nada sobre la referencia) | texto de la promesa tomado del mandato U-INV-MT del 21/09; cifras verificadas el 21/09 salvo las marcadas | **PROMESA** — la debe el capítulo 5; el capítulo 2 conserva solo 11 de 12 y 52 de 53 |
| P-6 | **PROMESA** (registrada 21/09/2026, hecho 4 de U-INV-MT): el capítulo 6 trata caso por caso los casos de nodo incompleto que la clase `generacion` puede ocultar | `ev2_reporte/salida/atribucion_fallas.md:305-315`: trazas en que `ver_nodo` del ancla devolvió un nodo con properties de encabezado o puntero y el agente declaró no encontrar el detalle (EV2F-001 y EV2F-007 en ambos refinados, EV2F-011 en KG-Base, EV2F-023 en KG-Reextraído); la regla las clasifica `generacion` porque el ancla fue consultada y declara el sub-diagnóstico «nodo-ancla sin el contenido pedido» como limitación (`regla_atribucion.md` §7); material para A0.3 / verificador causal con laudo humano | texto de la promesa tomado del mandato U-INV-MT del 21/09; anclas verificadas el 21/09 | **PROMESA** — la debe el capítulo 6 |
