...
- [ ] B5.3 (I, $0) A4/A5: `max_tokens` con reintento 16k→32k en el mismo pase; sub-chunking por ítems para TOs nuevos; no cerrar fase con errores reintentables; tope compartido entre clientes. **DESPACHADA el 05/09 — FE DE ERRATAS del registro previo (`docs/fe_erratas_despacho_UB53.md`): el registro la daba «despachada y en curso» desde el 04/09 y era falso al momento de la firma de B5.5; la condición del gate (el escalado no arranca sin su cierre) no cambia.**
