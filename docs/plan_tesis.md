...
- [ ] U-APP (propuesta de mesa, I, $0–1) **Deudas de la app detectadas en U-MIG-r1**, DOS ítems: (1) implementar la consecuencia app del laudo A1.6 (`89055c5`): `GraphAgentNeo4j` `modo='fulltext'` como default con `GraphIndex` in-memory como fallback declarado — hoy TODOS los grafos de la app (r1_vigente incluido) se sirven con el índice booleano, es decir el vigente promovido corre sobre el retriever NO promovido; (2) instrumentación de usage/costo por turno en el registro de sesiones (`app/sessions/*.jsonl` no persiste tokens — el costo del smoke se estimó desde la traza). Smoke propio con cita; sin tocar el cuarteto congelado.
