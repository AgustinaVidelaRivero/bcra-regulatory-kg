...
- [ ] A1.3 (H+I, ~$2) **Pre-registro sellado** de la ablación: config de retriever congelada por sha (analyzer, top-K, campos), semilla NUEVA de pares sintéticos (`sinteticas-faseA-v3`, USD ~2 de generación) para no iterar sobre material quemado, predicciones (brecha anti-léxica de KG-Refinado se achica ≥ X puntos; recall literal no baja; brecha visto-sin-consultado no cambia), tope. **Precisión laudada (cierre de A1.2):** el factor "tools v2" de A1.4 se sella en este pre-registro como **paquete** {tools v2 + specs JSON + las 2 líneas de prompt aprobadas} — un solo factor, no tres; el diff exacto de prompt queda citado por sha en el pre-registro.
...
- [ ] C1.7 Discusión: grounded ≠ correct; retrieval ≠ estructura; humano-en-el-loop (H7/H8, Motor 3 no validado); misreadings de métricas; límites (multi-hop, tablas, sujetos, familia del verificador); descopes. (S7–S8)
...
\* Vigencia de las tarifas: las estimaciones usan tarifas vigentes al 17/08; el precio intro
de Sonnet vence el 31/08; toda autorización de fase B re-verifica tarifa contra
documentación oficial y re-estima antes de correr; el buffer de USD 30 puede resultar corto
si las tarifas suben — se declara en la autorización correspondiente.
