...
| R5 | TextMineX: Data, Evaluation Framework and Ontology-guided LLM Pipeline for Humanitarian Mine Action | Chenyue Zhou, Gürkan Solmaz, Flavio Cirillo, Kiril Gashteovski, Jonathan Fürst | preprint arXiv, sin venue | v1 18/09/2025, v4 27/01/2026 | arXiv:2509.15098 · DOI 10.48550/arXiv.2509.15098 | A+B | Dataset + framework de evaluación + pipeline guiado por ontología, sobre reportes operativos del Cambodian Mine Action Centre en tripletas sujeto-relación-objeto. | Sí, el diseño más cercano al de esta tesis: tripletas anotadas por humanos más protocolo LLM-as-Judge con mitigación explícita de sesgo de posición en scoring sin referencia. +44,2% accuracy, −22,5% alucinación, +20,9% adherencia de formato con prompts alineados a ontología.  **AMPLIACIÓN (análisis verificado contra el PDF por mesa revisora, 03/09; Zhou et al. 2026, v4, NEC Labs Europe — ontología de 160 entidades / 86 relaciones, evaluación reference-based + juez LLM): (i) la entrada más comparable en FORMA a esta tesis, con contraste favorable en cada requisito — su gold es CIRCULAR (filtrado de las salidas de los dos modelos que luego ganan el ranking; un solo anotador no experto del dominio; sin kappa) vs claves selladas desde la fuente; su «alucinación» por substring es proxy de grounding, no de corrección. (ii) «conforme ≠ fiel» con datos ajenos: la conformidad de formato satura >95 % y la excluyen del score por no discriminar, mientras la alucinación de entidades va del 5 al 45 %. (iii) [INTERPRETACIÓN DE LA MESA — marca del tuneo 06/09: la afirmación «desplaza el error a la elección de relación» NO está en el paper; es lectura de la mesa sobre sus resultados, no hallazgo de los autores] sus 86 relaciones desplazan el error a la ELECCIÓN de relación — respaldo externo del vocabulario chico y del rechazo de los escalones 4-5. (iv) juez: la instrucción declarativa anti-sesgo no alcanza y la aleatorización estructural sí, y ni esa salva a los jueces débiles — respaldo del principio de calibración por ejemplos y estructura. (v) antipatrón citable: score combinado min-max (dependiente de qué modelos entran en la comparación) vs puntaje absoluto por criterio.** |
...
| R12 | **RETIRADA POR FABRICACIÓN** — la entrada previa («Measuring the sensitivity of LLM-based structured extraction…», «Martin Murin», arXiv:2606.05970) provenía de un resumen FABRICADO aportado por una instancia externa, ya declarado: el paper NO existe (verificado por mesa revisora contra arXiv). Ninguna referencia a él entra al repo; el resumen no commiteado fue eliminado. La pregunta que motivaba su selección (sensibilidad de la extracción a prompt/modelo/esquema) quedó respondida por instrumentos propios (escalera P1/P1″, ESQ-3b dos vueltas). | — | — | — | — | — | — | — |
...
| R14 | Improving Automatic Evaluation of LLMs in Biomedical Relation Extraction via LLMs-as-the-Judge | Md Tahmid Rahman Laskar, Israt Jahan, Elham Dolatabadi, Chun Peng, Enamul Hoque, Jimmy Huang | ACL 2025 Main (según Comments de arXiv) | 2025 (01/06) | arXiv:2506.00777 · DOI 10.48550/arXiv.2506.00777 | C | No publica KG: 8 LLMs como jueces evaluando respuestas de 5 LLMs sobre 3 datasets de extracción de relaciones biomédicas. Libera 36.000 muestras anotadas. | Advertencia directa sobre el juez de esta tesis: los jueces LLM arrancan por debajo del 50% de accuracy; el formato de salida estructurado da ~15% de mejora promedio y la adaptación de dominio suma más. Señala que las relaciones extraídas no siguen formato estándar.  **AMPLIACIÓN (análisis verificado contra el PDF por mesa revisora, 03/09): (1) cita para la exigencia del juez calibrado — jueces LLM de dominio general rinden <50 % EM en extracción de relaciones biomédicas (Claude-3-Haiku entre los peores en prosa), mejoran ~15 pp con salida estructurada del generador y solo superan a los cerrados con fine-tuning sobre adjudicación humana: refuerza el diseño ya planificado del juez de fidelidad calibrado contra adjudicación de la autora. (2) Su ablación reference-free (RMSE 1,53 con gold vs 3,19 sin) es la justificación citable de que EV2 sea reference-based con claves selladas. (3) Lectura crítica registrada: el fallo que miden es mayormente CONTEO sobre prosa, no emparejamiento semántico — el principio de esta tesis «el LLM clasifica, el código computa» lo resuelve por diseño; su T=1.0 en el juez y la falta de kappa inter-anotador limitan sus números.** |
...
- (Se retira de esta selección la entrada R12: fabricada — ver la fila R12 de §4. El id aparece también en `docs/consultas_busqueda_U-RW.md`, que es log histórico de consultas y no se reescribe. Tras el retiro, el argumento de línea queda en TRES papers reales verificados contra PDF: R3 (Colombo & Cambria — el hueco de fidelidad en KGs legales) y R14 (Laskar et al. — la necesidad del juez calibrado y del reference-based) y R5 (Zhou et al./TextMineX — el gold circular como contraste y «conforme ≠ fiel» con datos ajenos). NOTA de registro: el dictado del 03/09 pedía «fila nueva» para Zhou; TextMineX ya era R5 en este mapa, así que se AMPLIÓ la fila existente — sin duplicado.)
